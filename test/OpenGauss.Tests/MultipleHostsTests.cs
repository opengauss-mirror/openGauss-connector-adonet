...
using static OpenGauss.Tests.Support.MockState;
...
namespace OpenGauss.Tests
{
    ...
    public class MultipleHostsTests : TestBase
    {
        ...
        public async Task Connect_to_correct_host_pooled(string targetSessionAttributes, MockState[] servers, int expectedServer)
        {
            ...
            var connectionStringBuilder = new OpenGaussConnectionStringBuilder
            {
            ...
            string targetSessionAttributes, MockState[] servers, int expectedServer)
        {
            ...
            connectionString = new OpenGaussConnectionStringBuilder(connectionString)
            {
        ...
        public async Task Valid_host_not_found(string targetSessionAttributes, MockState[] servers)
        {
            ...
            var exception = Assert.ThrowsAsync<OpenGaussException>(async () => await OpenConnectionAsync(connectionString))!;
        ...
        public async Task Enlist_depends_on_session_attributes()
        {
            ...
            var defaultCsb = new OpenGaussConnectionStringBuilder
            {
            ...
            var primaryCsb = new OpenGaussConnectionStringBuilder(defaultConnectionString)
            {
                TargetSessionAttributes = "primary",
            };

            var standbyCsb = new OpenGaussConnectionStringBuilder(defaultConnectionString)
            {
                TargetSessionAttributes = "standby",
            };

            var preferPrimaryCsb = new OpenGaussConnectionStringBuilder(defaultConnectionString)
            {
                TargetSessionAttributes = "prefer-primary",
            };

            var preferStandbyCsb = new OpenGaussConnectionStringBuilder(defaultConnectionString)
            {
            ...
            OpenGaussConnector primaryConnector;
            OpenGaussConnector standbyConnector;
        ...
        public void All_hosts_are_down()
        {
            ...
            var connectionString = new OpenGaussConnectionStringBuilder
            {
            ...
            {
                Assert.That(aggregateException.InnerExceptions[i], Is.TypeOf<OpenGaussException>()
        ...
        public void TargetSessionAttributes_default_is_null()
            => Assert.That(new OpenGaussConnectionStringBuilder().TargetSessionAttributes, Is.Null);
        ...
        public void TargetSessionAttributes_invalid_throws()
            => Assert.Throws<ArgumentException>(() =>
                new OpenGaussConnectionStringBuilder
                {
        ...
        public async Task Connect_with_load_balancing()
        {
            ...
            OpenGaussConnector firstConnector;
            OpenGaussConnector secondConnector;
        ...
        public async Task Cluster_offline_state_on_connection_failure()
        {
            ...
            var csb = new OpenGaussConnectionStringBuilder(server.ConnectionString);
            await using var conn = new OpenGaussConnection(csb.ConnectionString);
        ...
        public async Task Cluster_offline_state_on_query_execution_pg_critical_failure()
        {
            ...
            var csb = new OpenGaussConnectionStringBuilder(postmaster.ConnectionString);
        ...
        public async Task Cluster_offline_state_on_query_execution_IOException()
        {
            ...
            var ex = Assert.ThrowsAsync<OpenGaussException>(() => conn.ExecuteNonQueryAsync("SELECT 1"))!;
        ...
        public async Task Cluster_unknown_state_on_query_execution_TimeoutException_with_cancellation_failure()
        {
            ...
            var ex = Assert.ThrowsAsync<OpenGaussException>(async () => await query)!;
            Assert.That(ex.InnerException, Is.TypeOf<TimeoutException>());
            Assert.That(conn.State, Is.EqualTo(ConnectionState.Open));
        ...
        public async Task Clear_pool_one_host_only_on_admin_shutdown()
        {
            ...
            var csb = new OpenGaussConnectionStringBuilder
            {
                Host = MultipleHosts(primaryPostmaster, standbyPostmaster),
                TargetSessionAttributes = "prefer-primary",
                ServerCompatibilityMode = ServerCompatibilityMode.NoTypeLoading,
        ...
        public void IntegrationTest([Values] bool loadBalancing, [Values] bool alwaysCheckHostState)
        {
            ...
            Task Client(OpenGaussConnectionStringBuilder csb, string targetSessionAttributes)
            {
                csb = csb.Clone();
