...
namespace OpenGauss.Tests
{
    public class SchemaTests : SyncOrAsyncTestBase
    {
        ...
        public async Task DataSourceInformation()
        {
            ...
            Assert.That(row["DataSourceProductName"], Is.EqualTo("OpenGauss"));
        ...
        public async Task DataTypes()
        {
            ...
            Assert.That(intRow["ProviderDbType"], Is.EqualTo((int)OpenGaussDbType.Integer));
            ...
            Assert.That(textRow["ProviderDbType"], Is.EqualTo((int)OpenGaussDbType.Text));
            ...
            Assert.That(numericRow["ProviderDbType"], Is.EqualTo((int)OpenGaussDbType.Numeric));
            ...
            Assert.That(intArrayRow["ProviderDbType"], Is.EqualTo((int)(OpenGaussDbType.Integer | OpenGaussDbType.Array)));
            ...
            Assert.That(intRangeRow["DataType"], Does.StartWith("OpenGauss.NET.Types.OpenGaussRange`1[[System.Int32"));
            Assert.That(intRangeRow["ProviderDbType"], Is.EqualTo((int)(OpenGaussDbType.Integer | OpenGaussDbType.Range)));
            ...
            Assert.That(enumRow["DataType"], Is.EqualTo("OpenGauss.Tests.SchemaTests+TestEnum"));
            ...
            Assert.That(compositeRow["DataType"], Is.EqualTo("OpenGauss.Tests.SchemaTests+TestComposite"));
            ...
            Assert.That(domainRow["ProviderDbType"], Is.EqualTo((int)OpenGaussDbType.Text));
        ...
        public async Task ParameterMarkerFormat()
        {
            ...
            conn.ExecuteNonQuery("CREATE TEMP TABLE pmf_data (int INTEGER)");
            conn.ExecuteNonQuery("INSERT INTO pmf_data (int) VALUES (4)");
            using var command = conn.CreateCommand();
            const string parameterName = "@p_int";
            command.CommandText = "SELECT * FROM pmf_data WHERE int=" +
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1831")]
...
DROP TABLE IF EXISTS primary_key_data;
CREATE TABLE primary_key_data (id INT, f1 INT);
ALTER TABLE primary_key_data ADD PRIMARY KEY (id);");
                string[] restrictions = { null!, null!, "primary_key_data" };
            ...
            {
                conn.ExecuteNonQuery("DROP TABLE IF EXISTS primary_key_data");
            }
        }
...
");
        ...
        //[Test, IssueLink("https://github.com/opengauss/opengauss/issues/1886")]
...
");
                ...
                string[] restrictions = { "opengauss_tests", "public", "types_table", null! };
        ...
        async Task<DataTable> GetSchema(OpenGaussConnection conn)
            => IsAsync ? await conn.GetSchemaAsync() : conn.GetSchema();

        async Task<DataTable> GetSchema(OpenGaussConnection conn, string collectionName)
            => IsAsync ? await conn.GetSchemaAsync(collectionName) : conn.GetSchema(collectionName);

        async Task<DataTable> GetSchema(OpenGaussConnection conn, string collectionName, string?[] restrictions)
