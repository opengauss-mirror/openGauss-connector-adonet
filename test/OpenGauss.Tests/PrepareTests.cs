...
namespace OpenGauss.Tests
{
    public class PrepareTests: TestBase
    {
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/3443")]
        public void Bug3443()
        {
            using var conn = OpenConnectionAndUnprepare();
            using var cmd = new OpenGaussCommand("SELECT 1", conn);
        ...
        public void Named_parameters()
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                using var command = new OpenGaussCommand("SELECT @a, @b", conn);
                command.Parameters.Add(new OpenGaussParameter("a", DbType.Int32));
                command.Parameters.Add(new OpenGaussParameter("b", 8));
                command.Prepare();
        ...
        public void Positional_parameters()
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                using var command = new OpenGaussCommand("SELECT $1, $2", conn);
                command.Parameters.Add(new OpenGaussParameter { DbType = DbType.Int32 });
                command.Parameters.Add(new OpenGaussParameter { Value = 8 });
                command.Prepare();
                command.Parameters[0].Value = 3;
                command.Parameters[1].Value = 5;
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1207")]
        ...
        public void Across_close_open_different_connector()
        {
            ...
            using var conn1 = new OpenGaussConnection(connString);
        ...
        public void Reuse_prepared_statement()
        {
            ...
            using (var cmd1 = new OpenGaussCommand("SELECT @p", conn1))
            {
            ...
            using (var cmd2 = new OpenGaussCommand("SELECT @p", conn1))
            {
        ...
        {
            using var conn = OpenConnectionAndUnprepare();
            using (var batch = new OpenGaussBatch(conn) { BatchCommands = { new("SELECT 1"), new("SELECT 2") } })
            {
                batch.Prepare();
        ...
        {
            using var conn = OpenConnectionAndUnprepare();
            using var cmd = new OpenGaussCommand("SELECT 1; SELECT 1", conn);
            cmd.Prepare();
            AssertNumPreparedStatements(conn, 1);
            cmd.ExecuteNonQuery();
            cmd.Unprepare();
        ...
        public void One_command_same_sql_auto_prepare()
        {
            ...
            for (var i = 0; i < 2 + 1; i++)
                sql.Append("SELECT 1;");
            using (var cmd = new OpenGaussCommand(sql.ToString(), conn))
                cmd.ExecuteNonQuery();
            AssertNumPreparedStatements(conn, 1);
        }
        ...
        public void One_command_same_sql_twice_with_params()
        {
            ...
            cmd.Parameters.Add("p1", OpenGaussDbType.Integer);
            cmd.Parameters.Add("p2", OpenGaussDbType.Integer);
            cmd.Prepare();
            AssertNumPreparedStatements(conn, 1);
        ...
        {
            using var conn = OpenConnectionAndUnprepare();
            using var cmd1 = new OpenGaussCommand("SELECT 1; SELECT 2", conn);
            using var cmd2 = new OpenGaussCommand("SELECT 2; SELECT 3", conn);
            cmd1.Prepare();
            cmd2.Prepare();
        ...
        public void Overloaded_sql()
        {
            ...
            {
                cmd.Parameters.Add("p", OpenGaussDbType.Integer);
                cmd.Prepare();
                Assert.That(cmd.IsPrepared, Is.True);
            }
            using (var cmd = new OpenGaussCommand("SELECT @p", conn))
            {
                cmd.Parameters.AddWithValue("p", OpenGaussDbType.Text, "foo");
                cmd.Prepare();
        ...
        {
            var connSettings = new OpenGaussConnectionStringBuilder(ConnectionString)
            {
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/2665")]
        public void Prepared_command_failure()
        {
            using var conn = OpenConnection();

            using (var command = new OpenGaussCommand("INSERT INTO test_table (id) VALUES (1)", conn))
        ...
        public void Same_sql_different_params()
        {
            ...
            using (var cmd = new OpenGaussCommand("SELECT @p", conn))
            {
                throw new NotImplementedException("Problem: currentl setting OpenGaussParameter.Value clears/invalidates...");
                cmd.Parameters.Add(new OpenGaussParameter("p", OpenGaussDbType.Integer));
                cmd.Prepare(true);

                cmd.Parameters[0].OpenGaussDbType = OpenGaussDbType.Text;
                Assert.That(cmd.IsPrepared, Is.False);
                cmd.Prepare(true);
                using (var crapCmd = new OpenGaussCommand("SELECT name,statement,parameter_types::TEXT[] FROM pg_prepared_statements", conn))
        ...
        */
        ...
        {
            using var conn = OpenConnection();
            var cmd = new OpenGaussCommand("sele", conn);
        ...
        {
            using var conn = OpenConnection();
            using var cmd1 = new OpenGaussCommand("SELECT @p1;", conn);
            using var cmd2 = new OpenGaussCommand("SELECT @p1; SELECT @p2;", conn);
            var p1 = new OpenGaussParameter("p1", OpenGaussDbType.Integer);
            var p21 = new OpenGaussParameter("p1", OpenGaussDbType.Text);
            var p22 = new OpenGaussParameter("p2", OpenGaussDbType.Text);
        ...
        {
            var builder = new OpenGaussConnectionStringBuilder(ConnectionString) { Multiplexing = true };
        ...
        OpenGaussConnection OpenConnectionAndUnprepare(string? connectionString = null)
        {
        ...
        OpenGaussConnection OpenConnectionAndUnprepare(OpenGaussConnectionStringBuilder csb)
            => OpenConnectionAndUnprepare(csb.ToString());

        void AssertNumPreparedStatements(OpenGaussConnection conn, int expected)
        ...
        void AssertNumPreparedStatements(OpenGaussConnection conn, int expected, string message)
        ...
        List<string> GetPreparedStatements(OpenGaussConnection conn)
        {
            var statements = new List<string>();
            using var cmd = new OpenGaussCommand("SELECT name FROM pg_prepared_statements WHERE statement NOT LIKE '%FROM pg_prepared_statement%'", conn);
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
