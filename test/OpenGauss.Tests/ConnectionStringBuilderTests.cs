...
namespace OpenGauss.Tests
{
    class ConnectionStringBuilderTests
    {
        ...
        public void Clear()
        {
            var builder = new OpenGaussConnectionStringBuilder { Host = "myhost" };
            builder.Clear();
            Assert.That(builder.Count, Is.EqualTo(0));
            Assert.That(builder["host"], Is.EqualTo(""));
        ...
        {
            var builder = new OpenGaussConnectionStringBuilder();
            Assert.That(builder.Port, Is.EqualTo(OpenGaussConnection.DefaultPort));
            builder.Port = 8;
            builder.Remove("Port");
            Assert.That(builder.Port, Is.EqualTo(OpenGaussConnection.DefaultPort));
        }
        ...
        {
            var builder = new OpenGaussConnectionStringBuilder();
            Assert.That(() => builder.ConnectionString = "Server=127.0.0.1;User Id=opengauss_tests;Pooling:false",
