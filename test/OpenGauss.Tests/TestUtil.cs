...
namespace OpenGauss.Tests
{
    public static class TestUtil
    {
        ...
        /// test database.
        /// </summary>
        public const string DefaultConnectionString = "Server=127.0.0.1;Port=15432;Username=gaussdb;Password=SZoscar55#;Database=postgres;Timeout=60;Command Timeout=60";
        ...
        public static void MinimumPgVersion(OpenGaussConnection conn, string minVersion, string? ignoreText = null)
        {
        ...
        public static void MaximumPgVersionExclusive(OpenGaussConnection conn, string maxVersion, string? ignoreText = null)
        {
        ...
        public static bool IsPgPrerelease(OpenGaussConnection conn)
            => ((string)conn.ExecuteScalar("SELECT version()")!).Contains("beta");

        public static void EnsureExtension(OpenGaussConnection conn, string extension, string? minVersion = null)
            => EnsureExtension(conn, extension, minVersion, async: false).GetAwaiter().GetResult();

        public static Task EnsureExtensionAsync(OpenGaussConnection conn, string extension, string? minVersion = null)
            => EnsureExtension(conn, extension, minVersion, async: true);

        static async Task EnsureExtension(OpenGaussConnection conn, string extension, string? minVersion, bool async)
        {
            if (minVersion != null)
        ...
        public static void IgnoreIfFeatureNotSupported(OpenGaussConnection conn, string testQuery)
        ...
        /// <param name="testQuery">The query to test for the feature.
        /// This query needs to fail with SqlState 0A000 (feature_not_supported) if the feature isn't present.</param>
        public static Task IgnoreIfFeatureNotSupportedAsync(OpenGaussConnection conn, string testQuery)
            => IgnoreIfFeatureNotSupported(conn, testQuery, async: true);

        static async Task IgnoreIfFeatureNotSupported(OpenGaussConnection conn, string testQuery, bool async)
        {
            try
            {
                if (async)
        ...
        public static async Task EnsurePostgis(OpenGaussConnection conn)
        {
            try
            {
        ...
        /// </summary>
        internal static Task<IAsyncDisposable> CreateTempTable(OpenGaussConnection conn, string columns, out string tableName)
        {
        ...
        /// drop it at the end of the test.
        /// </summary>
        internal static Task<IAsyncDisposable> CreateTempSchema(OpenGaussConnection conn, out string schemaName)
        {
        ...
        /// </returns>
        internal static Task<IAsyncDisposable> GetTempTableName(OpenGaussConnection conn, out string tableName)
        {
            tableName = "temp_table" + Interlocked.Increment(ref _tempTableCounter);
        ...
        /// </returns>
        internal static Task<IAsyncDisposable> GetTempViewName(OpenGaussConnection conn, out string viewName)
        {
        ...
        /// </returns>
        internal static IAsyncDisposable GetTempFunctionName(OpenGaussConnection conn, out string functionName)
        {
            functionName = "temp_func" + Interlocked.Increment(ref _tempFunctionCounter) + DateTimeOffset.Now.ToUnixTimeSeconds();
        ...
        /// An <see cref="IDisposable"/> to drop the function at the end of the test.
        /// </returns>
        internal static Task<IAsyncDisposable> GetTempTypeName(OpenGaussConnection conn, out string typeName)
        {
        ...
        sealed class DatabaseObjectDropper : IAsyncDisposable
        {
            ...
            internal DatabaseObjectDropper(OpenGaussConnection conn, string name, string type)
        ...
        internal static IDisposable CreateTempPool(string origConnectionString, out string tempConnectionString)
            => CreateTempPool(new OpenGaussConnectionStringBuilder(origConnectionString), out tempConnectionString);

        /// <summary>
        /// Creates a pool with a unique application name, usable for a single test, and returns an
        /// <see cref="IDisposable"/> to drop it at the end of the test.
        /// </summary>
        internal static IDisposable CreateTempPool(OpenGaussConnectionStringBuilder builder, out string tempConnectionString)
        {
        ...
        {
#if DEBUG
            OpenGaussCommand.EnableSqlRewriting = false;
            return new DeferredExecutionDisposable(() => OpenGaussCommand.EnableSqlRewriting = true);
#else
    ...
    public static class OpenGaussConnectionExtensions
    {
        public static int ExecuteNonQuery(this OpenGaussConnection conn, string sql, OpenGaussTransaction? tx = null)
        {
            var cmd = tx == null ? new OpenGaussCommand(sql, conn) : new OpenGaussCommand(sql, conn, tx);
            using (cmd)
                return cmd.ExecuteNonQuery();
        }

        public static object? ExecuteScalar(this OpenGaussConnection conn, string sql, OpenGaussTransaction? tx = null)
        {
            var cmd = tx == null ? new OpenGaussCommand(sql, conn) : new OpenGaussCommand(sql, conn, tx);
            using (cmd)
        ...
        public static async Task<int> ExecuteNonQueryAsync(this OpenGaussConnection conn, string sql, OpenGaussTransaction? tx = null,
        ...
        public static async Task<object?> ExecuteScalarAsync(this OpenGaussConnection conn, string sql, OpenGaussTransaction? tx = null,
            CancellationToken cancellationToken = default)
        {
            var cmd = tx == null ? new OpenGaussCommand(sql, conn) : new OpenGaussCommand(sql, conn, tx);
            using (cmd)
    ...
    public static class OpenGaussCommandExtensions
    {
        public static void WaitUntilCommandIsInProgress(this OpenGaussCommand command)
        {
