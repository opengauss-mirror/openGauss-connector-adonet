...
namespace OpenGauss.Tests
{
    public class NestedDataReaderTests : TestBase
    {
        ...
        public async Task Basic()
        {
            ...
            await using var command = new OpenGaussCommand(@"SELECT ARRAY[ROW(1, 2, 3), ROW(4, 5, 6)]
                                                    UNION ALL
        ...
        {
            await using var conn = await OpenConnectionAsync();
            await using var command = new OpenGaussCommand(@"SELECT ARRAY[ROW(1), ROW(), ROW('2'::TEXT, 3), ROW(4)]", conn);
            await using var reader = await command.ExecuteReaderAsync();
        ...
        {
            await using var conn = await OpenConnectionAsync();
            await using var command = new OpenGaussCommand(@"SELECT
                ARRAY[
                    ROW(
                        ARRAY[
        ...
        {
            await using var conn = await OpenConnectionAsync();
            await using var command = new OpenGaussCommand("SELECT ROW(1, ARRAY[ROW(2), ROW(3)])", conn);
            await using var reader = await command.ExecuteReaderAsync();
            await reader.ReadAsync();
            using var nestedReader = reader.GetData(0);
            Assert.That(nestedReader.Read(), Is.True);
        ...
        {
            await using var conn = await OpenConnectionAsync();
            await using var command = new OpenGaussCommand("SELECT ARRAY[]::RECORD[]", conn);
            await using var reader = await command.ExecuteReaderAsync();
            await reader.ReadAsync();
            using var nestedReader = reader.GetData(0);
            Assert.That(nestedReader.Read(), Is.False);
        ...
        public async Task Composite()
        {
            ...
            {
                await using var command = new OpenGaussCommand(sql, conn);
                await using var reader = await command.ExecuteReaderAsync();
                await reader.ReadAsync();
                using var nestedReader = reader.GetData(0);
                nestedReader.Read();
        ...
        public void GetBytes()
        {
            ...
            using var command = new OpenGaussCommand(@"SELECT ROW('\x010203'::BYTEA, NULL::BYTEA)", conn);
        ...
        {
            await using var conn = await OpenConnectionAsync();
            await using var command = new OpenGaussCommand(@"SELECT ROW(1) UNION ALL SELECT ROW(2) UNION ALL SELECT ROW(3)", conn);
            await using var reader = await command.ExecuteReaderAsync();
            Assert.That(await reader.ReadAsync(), Is.True);
