...
namespace OpenGauss.Tests
{
    ...
    public class ReaderNewSchemaTests : SyncOrAsyncTestBase
    {
        ...
        public async Task Allow_DBNull()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT nullable,non_nullable,8 FROM {table}", conn);
        ...
        {
            var dbName = new OpenGaussConnectionStringBuilder(ConnectionString).Database;
            using var conn = await OpenConnectionAsync();
        ...
        public async Task BaseColumnName()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT foo, foo AS foobar, 8 AS bar, 8, '8'::VARCHAR(10) FROM {table}", conn);
        ...
        public async Task BaseColumnName_with_column_aliases()
        {
            ...
            var cmd = new OpenGaussCommand("SELECT Cod as CodAlias, Descr as DescrAlias, Date, NULL AS Generated FROM data", conn);
        ...
        {
            var host = new OpenGaussConnectionStringBuilder(ConnectionString).Host;
            using var conn = await OpenConnectionAsync();
            await using var _ = await CreateTempTable(conn, "foo INTEGER", out var table);
        ...
        public async Task ColumnName()
        {
            ...
            // See https://github.com/opengauss/opengauss/issues/1676
            using (var conn = await OpenConnectionAsync())
            {
                ...
                using var command = new OpenGaussCommand($"SELECT col AS col_alias FROM {table}", conn);
        ...
        public async Task ColumnOrdinal()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT second,first FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task ColumnSize()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT bounded,unbounded,'a'::VARCHAR(10),'b'::VARCHAR FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task IsAutoIncrement()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT inc,non_inc,8 FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task IsAutoIncrement_identity()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT inc,identity,non_inc,8 FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
            Assert.That(columns[0].IsAutoIncrement, Is.True, "Serial not identified as autoincrement");
        ...
        public async Task IsKey()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT id,non_id,uniq,8 FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task IsKey_composite()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT id1,id2 FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
            Assert.That(columns[0].IsKey, Is.True);
        ...
        public async Task IsLong()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT long, non_long, 8 FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
...
CREATE TABLE {table} (bar INTEGER)");
            ...
            using var cmd = new OpenGaussCommand($"SELECT foo,bar FROM {view},{table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
...
CREATE UNIQUE INDEX idx_{table} ON {table} (non_id_second, non_id_third)");
            ...
            using var cmd = new OpenGaussCommand($"SELECT id,non_id,uniq,8,non_id_second,non_id_third FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task NumericPrecision()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT a,b,c,8.3::NUMERIC(8) FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task NumericScale()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT a,b,c,8.3::NUMERIC(8,5) FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task DataType()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT foo,8::INTEGER FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1305")]
        public async Task DataType_unknown_type()
        {
            using var conn = await OpenConnectionAsync();
            await using var _ = await CreateTempTable(conn, "foo INTEGER", out var table);

            using var cmd = new OpenGaussCommand($"SELECT foo::INTEGER FROM {table}", conn);
        ...
        public async Task DataType_with_composite()
        {
            ...
            using var cmd = new OpenGaussCommand("SELECT comp,'(4)'::some_composite FROM data", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task DataTypeName(string typeName, int? size = null, int? precision = null, int? scale = null)
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT foo,NULL::{typeName} FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
        ...
        public async Task DefaultValue()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT with_default,without_default,8 FROM {table}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
...
CREATE TABLE {table2} (foo INTEGER)");
            ...
            using var cmd = new OpenGaussCommand($"SELECT {table1}.foo,{table2}.foo FROM {table1},{table2}", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
            Assert.That(columns[0].ColumnName, Is.EqualTo("foo"));
            Assert.That(columns[0].BaseTableName, Does.StartWith("temp_table"));
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1553")]
        public async Task Domain_type()
        {
            ...
            using var cmd = new OpenGaussCommand("SELECT domain FROM data", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
            var columns = await GetColumnSchema(reader);
        ...
        [Test]
        public async Task OpenGaussDbType()
        {
            using var conn = await OpenConnectionAsync();
            await using var _ = await CreateTempTable(conn, "foo INTEGER", out var table);
            ...
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
            Assert.That(columns[0].OpenGaussDbType, Is.EqualTo(OpenGauss.NET.Types.OpenGaussDbType.Integer));
            Assert.That(columns[1].OpenGaussDbType, Is.EqualTo(OpenGauss.NET.Types.OpenGaussDbType.Integer));
        }

        [Test]
        public async Task OpenGaussDbType_extension()
        {
            using var conn = await OpenConnectionAsync();
            await EnsureExtensionAsync(conn, "hstore", "9.1");

            using var cmd = new OpenGaussCommand("SELECT NULL::HSTORE", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
            var columns = await GetColumnSchema(reader);
            // The full datatype name for PostGIS is public.geometry (unlike int4 which is in pg_catalog).
            Assert.That(columns[0].OpenGaussDbType, Is.EqualTo(OpenGauss.NET.Types.OpenGaussDbType.Hstore));
        }

        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1950")]
        public async Task No_resultset()
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("COMMIT", conn);
        ...
        public async Task IsAliased()
        {
            ...
            using var cmd = new OpenGaussCommand($"SELECT foo, foo AS bar, NULL AS foobar FROM {table}", conn);
            await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        ...
        async Task<ReadOnlyCollection<NET.Schema.OpenGaussDbColumn>> GetColumnSchema(OpenGaussDataReader reader)
