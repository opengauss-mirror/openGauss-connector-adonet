...
namespace OpenGauss.Tests
{
    class SqlQueryParserTests
    {
        ...
        {
            var parameters = new OpenGaussParameter[] { new(":p1", "foo"), new(":p2", "foo") };
         ...
         {
             var p = new OpenGaussParameter(":a.parameter", "foo");
         ...
         {
             var results = ParseCommand(sql, new OpenGaussParameter(":param", "foo"));
         ...
         {
             var p = new OpenGaussParameter(":param", "foo");
             var results = ParseCommand(sql, p);
             Assert.That(results.Single().PositionalParameters.Single(), Is.SameAs(p));
         }

         [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1177")]
         public void Parameter_gets_bound_non_ascii()
         {
             var p = new OpenGaussParameter("漢字", "foo");
...
2", TestName = "LineComment")]
        public void Parameter_does_not_get_bound(string sql)
        {
         ...
         {
             var parameters = new OpenGaussParameter[]
             {
         ...
         {
             var p = new OpenGaussParameter("p", DbType.String) { Direction = ParameterDirection.Output };
         ...
         public void Reduce_number_of_statements()
         {
             ...
             var cmd = new OpenGaussCommand("SELECT 1; SELECT 2");
        ...
        List<OpenGaussBatchCommand> ParseCommand(string sql, params OpenGaussParameter[] parameters)
            => ParseCommand(sql, parameters, standardConformingStrings: true);

        List<OpenGaussBatchCommand> ParseCommand(string sql, OpenGaussParameter[] parameters, bool standardConformingStrings)
        {
            var cmd = new OpenGaussCommand(sql);
