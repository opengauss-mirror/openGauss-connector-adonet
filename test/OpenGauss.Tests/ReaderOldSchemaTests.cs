...
CREATE TABLE {table} (
    ...
    CONSTRAINT data2_pkey PRIMARY KEY (field_pk1, field_pk2)
            ...
            using var command = new OpenGaussCommand($"SELECT * FROM {table}", conn);
...
CREATE OR REPLACE VIEW {view} (id, int2) AS SELECT id, int2 + int2 AS int2 FROM {table}");
            ...
            var command = new OpenGaussCommand($"SELECT * FROM {view}", conn);
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1027")]
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT 1::NUMERIC AS result", conn);
            using var reader = await cmd.ExecuteReaderAsync();
...
SELECT 1 AS some_other_column, 2";
        ...
        public async Task BaseColumnName()
        {
            ...
            var cmd = new OpenGaussCommand("SELECT Cod as CodAlias, Descr as DescrAlias, Date FROM bcn_data", conn);
        ...
        async Task<DataTable?> GetSchemaTable(OpenGaussDataReader dr) => IsAsync ? await dr.GetSchemaTableAsync() : dr.GetSchemaTable();
    }
}
