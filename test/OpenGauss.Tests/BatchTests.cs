...
using StatementType = OpenGauss.NET.StatementType;
...
namespace OpenGauss.Tests
{
    ...
    public class BatchTests : MultiplexingTestBase
    {
        ...
        public async Task Named_parameters()
        {
            ...
            await using var batch = new OpenGaussBatch(conn)
            {
                BatchCommands =
                {
        ...
        [Test]
        public async Task OpenGaussBatchCommand_StatementType()
        {
            await using var conn = await OpenConnectionAsync();
            await using var _ = await CreateTempTable(conn, "name TEXT", out var table);
        ...
        #endregion OpenGaussBatchCommand
...
LANGUAGE 'plpgsql'");
            ...
            // We use OpenGaussConnection.CreateBatch to test that the batch isn't recycled when referenced in an exception
...
LANGUAGE 'plpgsql'");
        ...
        {
            await using var conn = await OpenConnectionAsync();
            var batch = new OpenGaussBatch(conn) { BatchCommands = { new("SELECT 1") } };
