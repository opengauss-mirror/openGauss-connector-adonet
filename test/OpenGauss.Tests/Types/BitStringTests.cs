...
namespace OpenGauss.Tests.Types
{
    ...
    public class BitStringTests : MultiplexingTestBase
    {
        ...
        )
        {
            ...
            var p1 = new OpenGaussParameter("p1", OpenGaussDbType.Varbit);
            var p2 = new OpenGaussParameter("p2", OpenGaussDbType.Bit);
            var p3 = new OpenGaussParameter("p3", OpenGaussDbType.Varbit) {Value = bits};
        ...
        {
            using var conn = await OpenConnectionAsync();
            using (var cmd = new OpenGaussCommand($"SELECT B'{new string('0', 34)}'", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT @p::BIT(1), B'01'::BIT(2)", conn);
            const bool expected = true;
            var p = new OpenGaussParameter("p", OpenGaussDbType.Bit);
        ...
        {
            using var conn = await OpenConnectionAsync();
            using (var cmd = new OpenGaussCommand("SELECT B'01'::BIT(2)", conn))
        ...
        public async Task Array()
        {
            ...
            var p = new OpenGaussParameter("p", OpenGaussDbType.Array | OpenGaussDbType.Varbit) { Value = expected };
            cmd.Parameters.Add(p);
            p.Value = expected;
            using var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT @p::BIT(1)[]", conn);
            var expected = new[] { true, false };
            var p = new OpenGaussParameter("p", OpenGaussDbType.Array | OpenGaussDbType.Bit) {Value = expected};
            cmd.Parameters.Add(p);
            p.Value = expected;
            using var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT @p1::BIT VARYING", conn);
            var p = new OpenGaussParameter("p1", OpenGaussDbType.Bit);
            cmd.Parameters.Add(p);
            p.Value = "001q0";
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT 1::bit(100000)", conn);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SequentialAccess);
        ...
        public async Task Bitstring([Values(true, false)] bool prepareCommand)
        {
            using (var cmd = Conn.CreateCommand())
            {
                ...
                var output = new OpenGaussParameter() { ParameterName = "output", Direction = ParameterDirection.Output };
                cmd.Parameters.Add(output);
                cmd.Parameters.Add(new OpenGaussParameter("bs1", OpenGaussDbType.Bit) { Value = new BitString("1011") });
                cmd.Parameters.Add(new OpenGaussParameter("bs2", OpenGaussDbType.Bit, 1) { Value = true });
                cmd.Parameters.Add(new OpenGaussParameter("bs3", OpenGaussDbType.Bit, 1) { Value = false });
                cmd.Parameters.Add(new OpenGaussParameter("bs4", OpenGaussDbType.Bit, 2) { Value = new BitString("01") });
                cmd.Parameters.Add(new OpenGaussParameter("bs5", OpenGaussDbType.Varbit) { Value = new BitString("01") });
                cmd.Parameters.Add(new OpenGaussParameter("ba1", OpenGaussDbType.Varbit | OpenGaussDbType.Array) { Value = new BitString[] { new BitString("10"), new BitString("01") } });
                cmd.Parameters.Add(new OpenGaussParameter("ba2", OpenGaussDbType.Bit | OpenGaussDbType.Array, 1) { Value = new bool[] { true, false } });
                cmd.Parameters.Add(new OpenGaussParameter("ba3", OpenGaussDbType.Bit | OpenGaussDbType.Array, 1) { Value = new BitString[] { new BitString("1"), new BitString("0") } });
                if (prepareCommand)
                    cmd.Prepare();
                using (var reader = await cmd.ExecuteReaderAsync())
                {
