...
namespace OpenGauss.Tests.Types
{
    public class NumericTests : MultiplexingTestBase
    {
        ...
        public async Task Mapping()
        {
            ...
            cmd.Parameters.Add(new OpenGaussParameter("p1", OpenGaussDbType.Numeric) { Value = 8M });
            cmd.Parameters.Add(new OpenGaussParameter("p2", DbType.Decimal) { Value = 8M });
            cmd.Parameters.Add(new OpenGaussParameter("p3", DbType.VarNumeric) { Value = 8M });
            cmd.Parameters.Add(new OpenGaussParameter("p4", 8M));
        ...
        public async Task Read_overflow_is_safe()
        {
            ...
            using var cmd = new OpenGaussCommand(@"SELECT (0.20285714285714285714285714285)::numeric, generate_series FROM generate_series(1, 2)", conn);
        ...
        {
            var num = BigInteger.Parse(string.Join("", Enumerable.Range(0, 17).Select(i => ((i + 1) % 10).ToString())));
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT '0.1'::numeric, @p", conn);
