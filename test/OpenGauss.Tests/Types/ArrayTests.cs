...
namespace OpenGauss.Tests.Types
{
    ...
    public class ArrayTests : MultiplexingTestBase
    {
        ...
        public async Task Array_resolution()
        {
            ...
            {
                cmd.Parameters.AddWithValue("p", OpenGaussDbType.Array | OpenGaussDbType.Integer, DBNull.Value);
            ...
            using (var cmd = new OpenGaussCommand("SELECT @p", conn))
            {
                cmd.Parameters.Add(new OpenGaussParameter { ParameterName = "p", Value = new int[0] });
                using var reader = await cmd.ExecuteReaderAsync();
            ...
            conn.ReloadTypes();
            using (var cmd = new OpenGaussCommand("SELECT '{1, 3}'::INTEGER[]", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
                Assert.That(reader.GetDataTypeName(0), Is.EqualTo("integer[]"));
        ...
        public async Task Ints()
        {
            ...
            var p1 = new OpenGaussParameter("p1", OpenGaussDbType.Array | OpenGaussDbType.Integer);
            var p2 = new OpenGaussParameter { ParameterName = "p2", Value = expected };
            var p3 = new OpenGaussParameter<int[]>("p3", expected);
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT '{1, NULL, 2}'::integer[]", conn);
            using var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
        ...
        public async Task Empty_array()
        {
            ...
            cmd.Parameters.Add(new OpenGaussParameter("p", OpenGaussDbType.Array | OpenGaussDbType.Integer) { Value = new int[0] });
            var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
        ...
        public async Task Empty_multidimensional_array()
        {
            ...
            cmd.Parameters.AddWithValue("p", OpenGaussDbType.Array | OpenGaussDbType.Integer, expected);
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT ARRAY[[1], [2]]", conn);
        ...
        public async Task Read_null_as_non_nullable_array_throws()
        {
            ...
            var expected = new int?[] { 1, 5, null, 9 };
            cmd.Parameters.AddWithValue("p1", OpenGaussDbType.Array | OpenGaussDbType.Integer, expected);
        ...
        public async Task Zero_dimensional()
        {
            ...
            var p = new OpenGaussParameter("p", OpenGaussDbType.Array | OpenGaussDbType.Integer) { Value = expected };
            cmd.Parameters.Add(p);
            var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
...
#pragma warning disable 618 // OpenGaussDate is obsolete, remove in 7.0
        [Test, Description("Reads a one-dimensional array dates, both as DateTime and as the provider-specific OpenGaussDate")]
...
#pragma warning restore 618
        ...
        {
            using var conn = await OpenConnectionAsync();
            using (var cmd = new OpenGaussCommand("SELECT '[2:3]={ 8, 9 }'::INT[]", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
        ...
        public async Task Array_of_byte_arrays()
        {
            ...
            var p1 = new OpenGaussParameter("p1", OpenGaussDbType.Array | OpenGaussDbType.Bytea);
            var p2 = new OpenGaussParameter { ParameterName = "p2", Value = expected };
            cmd.Parameters.Add(p1);
            cmd.Parameters.Add(p2);
            p1.Value = expected;
        ...
        public async Task IList_generic()
        {
            ...
            var p1 = new OpenGaussParameter { ParameterName = "p1", Value = expected };
            var p2 = new OpenGaussParameter { ParameterName = "p2", Value = expected };
            cmd.Parameters.Add(p1);
            cmd.Parameters.Add(p2);
            using var reader = await cmd.ExecuteReaderAsync();
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/844")]
        ...
        public async Task Mixed_element_types()
        {
            ...
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT @p1", conn);
            cmd.Parameters.AddWithValue("p1", OpenGaussDbType.Array | OpenGaussDbType.Integer, mixedList);
        ...
        public async Task Jagged_arrays_not_supported()
        {
            ...
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT @p1", conn);
            cmd.Parameters.AddWithValue("p1", OpenGaussDbType.Array | OpenGaussDbType.Integer, jagged);
            Assert.That(async () => await cmd.ExecuteNonQueryAsync(), Throws.Exception
                .TypeOf<Exception>()
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/1546")]
        public void Generic_List_get_OpenGaussDbType()
        {
        ...
        public async Task Array_of_domain()
        {
            ...
            conn.ReloadTypes();
            using var cmd = new OpenGaussCommand("SELECT @p1::posint[], @p2::posint[][]", conn);
            var oneDim = new[] { 1, 3, 5, 9 };
            var twoDim = new[,] { { 1, 3 }, { 5, 9 } };
            cmd.Parameters.AddWithValue("p1", OpenGaussDbType.Integer | OpenGaussDbType.Array, oneDim);
            cmd.Parameters.AddWithValue("p2", OpenGaussDbType.Integer | OpenGaussDbType.Array, twoDim);
        ...
        public async Task Domain_of_array()
        {
            ...
            conn.ReloadTypes();
            using var cmd = new OpenGaussCommand("SELECT @p1::int_array_1d, @p2::int_array_2d", conn);
            var oneDim = new[] { 1, 3, 5, 9 };
            var twoDim = new[,] { { 1, 3 }, { 5, 9 } };
            cmd.Parameters.AddWithValue("p1", OpenGaussDbType.Integer | OpenGaussDbType.Array, oneDim);
            cmd.Parameters.AddWithValue("p2", OpenGaussDbType.Integer | OpenGaussDbType.Array, twoDim);
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/3417")]
        ...
        {
            using var cmd = new OpenGaussCommand("SELECT @p", conn);
            cmd.Parameters.Add(new OpenGaussParameter { ParameterName = "p", Value = value });
