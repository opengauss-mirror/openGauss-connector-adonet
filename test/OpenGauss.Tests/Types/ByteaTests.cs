...
namespace OpenGauss.Tests.Types
{
    ...
    public class ByteaTests : MultiplexingTestBase
    {
        ...
        public async Task Roundtrip()
        {
            ...
            var p1 = new OpenGaussParameter("p1", OpenGaussDbType.Bytea);
            var p2 = new OpenGaussParameter("p2", DbType.Binary);
            var p3 = new OpenGaussParameter { ParameterName = "p3", Value = expected };
            Assert.That(p3.OpenGaussDbType, Is.EqualTo(OpenGaussDbType.Bytea));
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT @p::BYTEA", conn);
            var expected = new byte[conn.Settings.WriteBufferSize + 100];
            for (var i = 0; i < expected.Length; i++)
                expected[i] = 8;
            cmd.Parameters.Add(new OpenGaussParameter("p", OpenGaussDbType.Bytea) { Value = expected });
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT :val::BYTEA", conn);
            var expected = new byte[0];
            cmd.Parameters.Add("val", OpenGaussDbType.Bytea);
            cmd.Parameters["val"].Value = expected;
            var result = (byte[]?)await cmd.ExecuteScalarAsync();
            Assert.That(result, Is.EqualTo(expected));
        }

        [Test, Description("Tests that bytea values are truncated when the OpenGaussParameter's Size is set")]
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new OpenGaussCommand("SELECT @bytes", conn);
        ...
        public async Task ArraySegment()
        {
            ...
            using (var cmd = new OpenGaussCommand("select :bytearr", conn))
            {
                ...
                // Small value, should be written normally through the OpenGaussBuffer
        ...
        [IssueLink("https://github.com/opengauss/opengauss/issues/654")]
