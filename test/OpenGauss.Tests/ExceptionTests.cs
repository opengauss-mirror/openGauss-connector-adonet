...
namespace OpenGauss.Tests
{
    public class ExceptionTests : TestBase
    {
        ...
        public void Basic()
        {
            using var conn = OpenConnection(new OpenGaussConnectionStringBuilder(ConnectionString)
            {
            ...
            Assert.That(exString, Does.StartWith("OpenGauss.NET.PostgresException (0x80004005): 12345: testexception"));
...
$$ LANGUAGE 'plpgsql';");
        ...
        {
            var builder = new OpenGaussConnectionStringBuilder(ConnectionString) { IncludeErrorDetail = true };
...
$$ LANGUAGE 'plpgsql';");
        ...
        public void DataTypeName_is_populated()
        {
            ...
            try
            {
                var command = new OpenGaussCommand(dropDomain, conn);
                command.ExecuteNonQuery();

                command = new OpenGaussCommand(createDomain, conn);
                command.ExecuteNonQuery();

                command = new OpenGaussCommand(castStatement, conn);
        ...
        [Test]
        public void OpenGaussException_with_async()
        {
            using var conn = OpenConnection();
