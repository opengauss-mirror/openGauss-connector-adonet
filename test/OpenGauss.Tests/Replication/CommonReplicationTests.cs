...
using OpenGauss.NET.Replication.TestDecoding;
...
namespace OpenGauss.Tests.Replication
{
        ...
        public Task Replication_survives_pauses_longer_than_wal_sender_timeout()
            => SafeReplicationTest(
                async (slotName, tableName) =>
                {
                    ...
                    await using var rc = await OpenReplicationConnectionAsync(new OpenGaussConnectionStringBuilder(ConnectionString));
        ...
        public Task Synchronous_replication()
            => SafeReplicationTest(
                async (slotName, tableName) =>
                {
                    ...
                    if (synchronousStandbyNames != "opengauss_test_sync_standby")
                        TestUtil.IgnoreExceptOnBuildServer("Ignoring because synchronous_standby_names isn't 'opengauss_test_sync_standby'");
    ...
    ");
                    ...
                    await using var rc = await OpenReplicationConnectionAsync(new OpenGaussConnectionStringBuilder(ConnectionString)
                    {
                        ...
                        ApplicationName = "opengauss_test_sync_standby",
                    ...
                    {
                        await using var insertConn = await OpenConnectionAsync(new OpenGaussConnectionStringBuilder(ConnectionString)
                        {
                    ...
                    {
                        await using var insertConn = OpenConnection(new OpenGaussConnectionStringBuilder(ConnectionString)
                        {
                    ...
                    static async IAsyncEnumerable<(OpenGaussLogSequenceNumber Lsn, string? MessageData)> ParseMessages(
                    ...
                    async Task<OpenGaussLogSequenceNumber> GetCommitLsn(string valueString)
                    {
                        if (typeof(TConnection) == typeof(PhysicalReplicationConnection))
        ...
        [Test, IssueLink("https://github.com/opengauss/opengauss/issues/3534")]
        public Task Bug3534()
            => SafeReplicationTest(
                async (slotName, _) =>
                {
        ...
        async IAsyncEnumerable<ReplicationMessage> StartReplication(TConnection connection, string slotName,
            OpenGaussLogSequenceNumber xLogPos, [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            if (typeof(TConnection) == typeof(PhysicalReplicationConnection))
            {
