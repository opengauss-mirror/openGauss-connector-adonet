    ...
    public class PhysicalReplicationTests : SafeReplicationTestBase<PhysicalReplicationConnection>
    {
        ...
        public Task CreateReplicationSlot()
            => SafeReplicationTest(
                async (slotName, _) =>
                {
                    ...
                    using var cmd =
                        new OpenGaussCommand($"SELECT * FROM pg_replication_slots WHERE slot_name = '{slot.Name}'",
                            c);
        ...
        public Task Replication_with_slot()
            => SafeReplicationTest(
                ...
                {
                    // var messages = new ConcurrentQueue<(OpenGaussLogSequenceNumber WalStart, OpenGaussLogSequenceNumber WalEnd, byte[] data)>();
