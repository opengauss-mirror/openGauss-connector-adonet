        ...
        public Task CreateLogicalReplicationSlot(bool temporary)
            => SafeReplicationTest(
                async (slotName, _) =>
                {
                    ...
                    using var cmd =
                        new OpenGaussCommand($"SELECT * FROM pg_replication_slots WHERE slot_name = '{options.SlotName}'",
                            c);
                    ...
                    if (c.PostgreSqlVersion >= Version.Parse("9.6"))
                        Assert.That(reader.GetFieldValue<OpenGaussLogSequenceNumber>(reader.GetOrdinal("confirmed_flush_lsn")),
            ...
            => SafeReplicationTest(
                async (slotName, tableName) =>
                {
                    ...
                    await using (var transaction = c.BeginTransaction(IsolationLevel.RepeatableRead))
                    {
                        ...
                        using var cmd = new OpenGaussCommand($"SELECT value FROM {tableName}", c, transaction);
                        await using var reader = await cmd.ExecuteReaderAsync();
                        Assert.That(reader.Read, Is.True);
