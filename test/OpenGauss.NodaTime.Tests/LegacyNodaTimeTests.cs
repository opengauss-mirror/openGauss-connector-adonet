...
namespace OpenGauss.NodaTime.NET.Tests
{
    ...
    public class LegacyNodaTimeTests : TestBase
    {
        ...
        public async Task Timestamp_read_infinity()
        {
            await using var conn = await OpenConnectionAsync();
            await using var cmd =
                new OpenGaussCommand("SELECT 'infinity'::timestamp without time zone, '-infinity'::timestamp without time zone", conn);
            await using var reader = await cmd.ExecuteReaderAsync();
            await reader.ReadAsync();
        ...
        static OpenGaussParameter[] TimestamptzParameters
        {
            get
            {
                ...
                return new OpenGaussParameter[]
                {
                    ...
                    new() { Value = instance.InZone(DateTimeZoneProviders.Tzdb["America/New_York"]), OpenGaussDbType = OpenGaussDbType.TimestampTz },
                    new() { Value = instance.WithOffset(Offset.FromHours(1)), OpenGaussDbType = OpenGaussDbType.TimestampTz }
                };
            }
        }
        ...
        public async Task TimeTz()
        {
            ...
            cmd.Parameters.Add(new OpenGaussParameter("p4", dateTimeOffset));
        ...
        {
            var conn = new OpenGaussConnection(connectionString ?? ConnectionString);
...
#else
            Assert.Ignore(
                "Legacy NodaTime tests rely on the OpenGauss.EnableLegacyTimestampBehavior AppContext switch and can only be run in DEBUG builds");
#endif
