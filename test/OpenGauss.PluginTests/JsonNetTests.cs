...
{
    /// <summary>
    /// Tests for the OpenGauss.Json.NET mapping plugin
    /// </summary>
    [NonParallelizable]
    [TestFixture(OpenGaussDbType.Jsonb)]
    [TestFixture(OpenGaussDbType.Json)]
    public class JsonNetTests : TestBase
    {
        ...
        public void Roundtrip_object()
        {
            ...
            cmd.Parameters.Add(new OpenGaussParameter("p1", _opengaussDbType) { Value = expected });
            cmd.Parameters.Add(new OpenGaussParameter<Foo>
            {
                ParameterName = "p2", OpenGaussDbType = _opengaussDbType, TypedValue = expected
            });
        ...
        {
            using var conn = OpenConnection();
            using var cmd = new OpenGaussCommand($@"SELECT '[1, 2, 3]'::{_pgTypeName}", conn);
            using var reader = cmd.ExecuteReader();
            reader.Read();
        ...
        public void Roundtrip_string_types()
        {
            ...
            var expectedString = _opengaussDbType.Equals(OpenGaussDbType.Jsonb) ? "{\"p\": 1}"
                                    : "{\"p\":1}";

            using var conn = OpenConnection();
            using var cmd = new OpenGaussCommand(@"SELECT @p1, @p2, @p3", conn);

            cmd.Parameters.Add(new OpenGaussParameter<string>("p1", _opengaussDbType) { Value = expected });
            cmd.Parameters.Add(new OpenGaussParameter<char[]>("p2", _opengaussDbType) { Value = expected.ToCharArray() });
            cmd.Parameters.Add(new OpenGaussParameter<byte[]>("p3", _opengaussDbType) { Value = Encoding.ASCII.GetBytes(expected) });
        ...
        public void Roundtrip_ArraySegment()
        {
            ...
            cmd.Parameters.Add(new OpenGaussParameter<ArraySegment<char>>("p1", _opengaussDbType) { Value = new ArraySegment<char>(expected.ToCharArray()) });
        ...
        public void Roundtrip_JObject()
        {
            ...
            cmd.Parameters.Add(new OpenGaussParameter("p", _opengaussDbType) { Value = expected });
            using var reader = cmd.ExecuteReader();
            reader.Read();
        ...
        [Test, Ignore("https://github.com/opengauss/opengauss/issues/2568")]
        ...
        public void Roundtrip_clr_array()
        {
            ...
            using var cmd = new OpenGaussCommand($@"SELECT @p::{_pgTypeName}", conn);
            cmd.Parameters.AddWithValue("p", expected);
            using var reader = cmd.ExecuteReader();
            reader.Read();
        ...
        void RoundtripCustomSerializerSettings(bool asJsonb)
        {
            ...
            using var cmd = new OpenGaussCommand($@"SELECT @p::{_pgTypeName}, @p::text", conn);
            cmd.Parameters.AddWithValue("p", expected);
            using var reader = cmd.ExecuteReader();
            reader.Read();
        ...
        public void Bug3464()
        {
            ...
            cmd.Parameters.AddWithValue("p1", expected).OpenGaussDbType = _opengaussDbType;
            cmd.Parameters.AddWithValue("p2", expected).OpenGaussDbType = _opengaussDbType;
        ...
        public JsonNetTests(OpenGaussDbType opengaussDbType)
        {
            _opengaussDbType = opengaussDbType;
            _pgTypeName = opengaussDbType.ToString().ToLower();
        }
    }
}
