...
namespace OpenGauss.NET.Util
{
    static class Statics
    {
        ...
        static Statics()
        {
            LegacyTimestampBehavior = AppContext.TryGetSwitch("OpenGauss.NET.EnableLegacyTimestampBehavior", out var enabled) && enabled;
            DisableDateTimeInfinityConversions = AppContext.TryGetSwitch("OpenGauss.NET.DisableDateTimeInfinityConversions", out enabled) && enabled;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static T Expect<T>(IBackendMessage msg, OpenGaussConnector connector)
        {
            if (msg is T asT)
                return asT;

            throw connector.Break(
                new OpenGaussException($"Received backend message {msg.Code} while expecting {typeof(T).Name}. " +
                                    "Please file a bug."));
        }
    ...
    static class PGUtil
    {
        ...
        internal static void ValidateBackendMessageCode(BackendMessageCode code)
        {
            ...
            case BackendMessageCode.RowDescription:
                return;
            default:
                throw new OpenGaussException("Unknown message code: " + code);
            }
        }
    ...
    /// </summary>
    public readonly struct OpenGaussTimeout
    {
        readonly DateTime _expiration;

        internal static OpenGaussTimeout Infinite = new(TimeSpan.Zero);

        internal OpenGaussTimeout(TimeSpan expiration)
        ...
        internal void CheckAndApply(OpenGaussConnector connector)
        {
            if (!IsSet)
                return;
