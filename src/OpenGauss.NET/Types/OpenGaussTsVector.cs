...
namespace OpenGauss.NET.Types
{
    ...
    /// </summary>
    public sealed class OpenGaussTsVector : IEnumerable<OpenGaussTsVector.Lexeme>
    {
        readonly List<Lexeme> _lexemes;

        internal OpenGaussTsVector(List<Lexeme> lexemes, bool noCheck = false)
        {
            if (noCheck)
            {
                _lexemes = lexemes;
        ...
        /// <returns></returns>
        public static OpenGaussTsVector Parse(string value)
        {
            if (value == null)
            ...
            Finish:
            return new OpenGaussTsVector(lexemes);
        }
