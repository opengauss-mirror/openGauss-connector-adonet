...
namespace OpenGauss.NET
{
    ...
    /// a PostgreSQL table. Initiated by <see cref="OpenGaussConnection.BeginRawBinaryCopy(string)"/>
    /// </summary>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public sealed class OpenGaussRawCopyStream : Stream, ICancelable
    {
        #region Fields and Properties

        OpenGaussConnector _connector;
        OpenGaussReadBuffer _readBuf;
        OpenGaussWriteBuffer _writeBuf;
        ...
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(OpenGaussRawCopyStream));

        #endregion

        #region Constructor / Initializer

        internal OpenGaussRawCopyStream(OpenGaussConnector connector)
        {
            ...
            try
            {
                ...
                else
                {
                    if (!_isConsumed)
                    {
                        ...
                        {
                            Log.Debug($"Caught an exception while disposing the {nameof(OpenGaussRawCopyStream)}, indicating that it was cancelled.", e, _connector.Id);
                        }
                        catch (Exception e)
                        {
                            Log.Error($"Caught an exception while disposing the {nameof(OpenGaussRawCopyStream)}.", e, _connector.Id);
                        }
                    }
                }
            }
...
#pragma warning restore CS8625
        ...
        {
            if (_isDisposed) {
                throw new ObjectDisposedException(nameof(OpenGaussRawCopyStream), "The COPY operation has already ended.");
            }
        }
    ...
    /// <summary>
    /// Writer for a text import, initiated by <see cref="OpenGaussConnection.BeginTextImport(string)"/>.
    /// </summary>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public sealed class OpenGaussCopyTextWriter : StreamWriter, ICancelable
    {
        internal OpenGaussCopyTextWriter(OpenGaussConnector connector, OpenGaussRawCopyStream underlying) : base(underlying)
        {
        ...
        /// </summary>
        public void Cancel()
            => ((OpenGaussRawCopyStream)BaseStream).Cancel();
        ...
        {
            using (NoSynchronizationContextScope.Enter())
                return ((OpenGaussRawCopyStream)BaseStream).CancelAsync();
        }
    ...
    /// <summary>
    /// Reader for a text export, initiated by <see cref="OpenGaussConnection.BeginTextExport(string)"/>.
    /// </summary>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public sealed class OpenGaussCopyTextReader : StreamReader, ICancelable
    {
        internal OpenGaussCopyTextReader(OpenGaussConnector connector, OpenGaussRawCopyStream underlying) : base(underlying)
        {
            if (underlying.IsBinary)
