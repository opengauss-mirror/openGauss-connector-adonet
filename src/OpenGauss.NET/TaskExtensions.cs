...
namespace OpenGauss.NET
{
    static class TaskExtensions
    {
        ...
        internal static async Task<T> WithTimeout<T>(this Task<T> task, OpenGaussTimeout timeout)
        {
            if (!timeout.IsSet)
                return await task;
        ...
        internal static Task<T> WithCancellationAndTimeout<T>(this Task<T> task, OpenGaussTimeout timeout, CancellationToken cancellationToken)
        ...
        /// <param name="timeout">How much time to allow <paramref name="task"/> to complete before throwing a <see cref="TimeoutException"/></param>
        /// <returns>An awaitable task that represents the original task plus the timeout</returns>
        internal static async Task WithTimeout(this Task task, OpenGaussTimeout timeout)
        {
            if (!timeout.IsSet)
            {
                await task;
        ...
        internal static Task WithCancellationAndTimeout(this Task task, OpenGaussTimeout timeout, CancellationToken cancellationToken)
            => task.WithCancellation(cancellationToken).WithTimeout(timeout);
#endif

        internal static async Task ExecuteWithTimeout(Func<CancellationToken, Task> func, OpenGaussTimeout timeout, CancellationToken cancellationToken)
        {
        ...
        internal static async Task<TResult> ExecuteWithTimeout<TResult>(Func<CancellationToken, Task<TResult>> func, OpenGaussTimeout timeout, CancellationToken cancellationToken)
        {
            CancellationTokenSource? combinedCts = null;
            try
            {
        ...
        static CancellationToken GetCombinedCancellationToken(ref CancellationTokenSource? combinedCts, OpenGaussTimeout timeout, CancellationToken cancellationToken)
        {
