...
namespace OpenGauss.NET.BackendMessages
{
    ...
    class ErrorOrNoticeMessage
    {
        ...
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(ErrorOrNoticeMessage));

        // ReSharper disable once FunctionComplexityOverflow
        internal static ErrorOrNoticeMessage Load(OpenGaussReadBuffer buf, bool includeDetail)
        {
            ...
            while (true)
            {
                ...
                case ErrorFieldTypeCode.Detail:
                    ...
                    if (!includeDetail && !string.IsNullOrEmpty(detail))
                        detail = $"Detail redacted as it may contain sensitive data. Specify '{OpenGaussConnectionStringBuilder.IncludeExceptionDetailDisplayName}' in the connection string to include this information.";
                    break;
            ...
            End:
            if (severity == null)
                throw new OpenGaussException("Severity not received in server error message");
            if (code == null)
                throw new OpenGaussException("Code not received in server error message");
            if (message == null)
                throw new OpenGaussException("Message not received in server error message");
