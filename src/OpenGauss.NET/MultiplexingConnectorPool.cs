...
namespace OpenGauss.NET
{
    ...
    {
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(MultiplexingConnectorPool));
        ...
        readonly ChannelReader<OpenGaussCommand> _multiplexCommandReader;
        internal ChannelWriter<OpenGaussCommand> MultiplexCommandWriter { get; }
        ...
        internal MultiplexingConnectorPool(
            OpenGaussConnectionStringBuilder settings, string connString, MultiHostConnectorPool? parentPool = null)
            : base(settings, connString, parentPool)
        {
            ...
            var multiplexCommandChannel = Channel.CreateBounded<OpenGaussCommand>(
        ...
        /// 2. Cause any connection exceptions (e.g. bad username) to be thrown from OpenGaussConnection.Open
        /// 3. Start the multiplexing write loop after we've made sure that's the exact pool we're going to use
        /// </summary>
        internal async Task BootstrapMultiplexing(OpenGaussConnection conn, OpenGaussTimeout timeout, bool async, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var __ = Task.Run(MultiplexingWriteLoop, CancellationToken.None)
                    .ContinueWith(t =>
                    {
                        ...
                        Log.Error("Exception in multiplexing write loop, this is an OpenGauss bug, please file an issue.",
                                t.Exception!);
        ...
        async Task MultiplexingWriteLoop()
        {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        connector = await OpenNewConnector(
                            command.Connection!,
                            new OpenGaussTimeout(TimeSpan.FromSeconds(Settings.Timeout)),
                            async: true,
            ...
            bool WriteCommand(OpenGaussConnector connector, OpenGaussCommand command, ref MultiplexingStats stats)
            {
            ...
            void Flush(OpenGaussConnector connector, ref MultiplexingStats stats)
            {
            ...
            void FailWrite(OpenGaussConnector connector, Exception exception)
            {
            ...
            static void CompleteWrite(OpenGaussConnector connector, ref MultiplexingStats stats)
            {
                ...
                OpenGaussEventSource.Log.MultiplexingBatchSent(stats.NumCommands, stats.Stopwatch);
            }
