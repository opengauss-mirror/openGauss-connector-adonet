...
namespace OpenGauss.NET
{
    ...
    /// a PostgreSQL table. Initiated by <see cref="OpenGaussConnection.BeginBinaryExport(string)"/>
    /// </summary>
    public sealed class OpenGaussBinaryExporter : ICancelable
    {
        #region Fields and Properties

        OpenGaussConnector _connector;
        OpenGaussReadBuffer _buf;
        ...
        OpenGaussTypeHandler?[] _typeHandlerCache;

        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(OpenGaussBinaryExporter));
        ...
        internal OpenGaussBinaryExporter(OpenGaussConnector connector)
        {
        ...
        internal async Task Init(string copyToCommand, bool async, CancellationToken cancellationToken = default)
        {
            ...
            _typeHandlerCache = new OpenGaussTypeHandler[NumColumns];
        ...
        async Task ReadHeader(bool async)
        {
            ...
            var headerLen = OpenGaussRawCopyStream.BinarySignature.Length + 4 + 4;
            await _buf.Ensure(headerLen, async);

            if (OpenGaussRawCopyStream.BinarySignature.Any(t => _buf.ReadByte() != t))
                throw new OpenGaussException("Invalid COPY binary signature at beginning!");
        ...
        /// corruption will occur. If in doubt, use <see cref="Read{T}(OpenGaussDbType)"/> to manually
        /// specify the type.
        /// </typeparam>
        ...
        /// <paramref name="type"/> must be specified as <see cref="OpenGaussDbType.Jsonb"/>.
        /// </param>
        /// <typeparam name="T">The .NET type of the column to be read.</typeparam>
        /// <returns>The value of the column</returns>
        public T Read<T>(OpenGaussDbType type) => Read<T>(type, false).GetAwaiter().GetResult();
        ...
        /// <typeparam name="T">The .NET type of the column to be read.</typeparam>
        /// <returns>The value of the column</returns>
        public ValueTask<T> ReadAsync<T>(OpenGaussDbType type, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
                return Read<T>(type, true, cancellationToken);
        }

        ValueTask<T> Read<T>(OpenGaussDbType type, bool async, CancellationToken cancellationToken = default)
        {
            CheckDisposed();
            if (_column == -1 || _column == NumColumns)
                throw new InvalidOperationException("Not reading a row");
            ...
            var handler = _typeHandlerCache[_column];
            if (handler == null)
                handler = _typeHandlerCache[_column] = _typeMapper.ResolveByOpenGaussDbType(type);

            return DoRead<T>(handler, async, cancellationToken);
        }

        async ValueTask<T> DoRead<T>(OpenGaussTypeHandler handler, bool async, CancellationToken cancellationToken = default)
        {
            try
            {
                using var registration = _connector.StartNestedCancellableOperation(cancellationToken);
...
#pragma warning restore CS8653
        ...
        async ValueTask DisposeAsync(bool async)
        {
            ...
            if (!_isConsumed && !_connector.IsBroken)
            {
                ...
                {
                    Log.Debug($"Caught an exception while disposing the {nameof(OpenGaussBinaryExporter)}, indicating that it was cancelled.", e, _connector.Id);
                }
                catch (Exception e)
                {
                    Log.Error($"Caught an exception while disposing the {nameof(OpenGaussBinaryExporter)}.", e, _connector.Id);
                }
            }
