...
namespace OpenGauss.NET
{
    ...
    /// Can be retrieved using <see cref="OpenGaussDataReader.GetData(int)"/> or
    /// <see cref="OpenGaussNestedDataReader.GetData(int)"/>.
    /// </summary>
    public sealed class OpenGaussNestedDataReader : DbDataReader
    {
        readonly OpenGaussDataReader _outermostReader;
        ulong _uniqueOutermostReaderRowId;
        readonly OpenGaussNestedDataReader? _outerNestedReader;
        OpenGaussNestedDataReader? _cachedFreeNestedDataReader;
        ...
        readonly struct ColumnInfo
        {
            ...
            public readonly OpenGaussTypeHandler TypeHandler;

            public ColumnInfo(uint typeOid, int bufferPos, OpenGaussTypeHandler typeHandler)
            {
                TypeOid = typeOid;
                BufferPos = bufferPos;
        ...
        OpenGaussReadBuffer Buffer => _outermostReader.Buffer;
        ConnectorTypeMapper TypeMapper => _outermostReader.Connector.TypeMapper;

        internal OpenGaussNestedDataReader(OpenGaussDataReader outermostReader, OpenGaussNestedDataReader? outerNestedReader,
        ...
        public new OpenGaussNestedDataReader GetData(int ordinal)
        {
            ...
            {
                reader = new OpenGaussNestedDataReader(_outermostReader, this, _uniqueOutermostReaderRowId, _depth + 1, compositeType);
            }
            if (isArray)
                reader.InitArray();
            else
        ...
        (OpenGaussTypeHandler Handler, int Length) CheckRowAndColumnAndSeek(int ordinal)
        {
            var column = CheckRowAndColumn(ordinal);
