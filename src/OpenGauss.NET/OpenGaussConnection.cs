...
namespace OpenGauss.NET
{
    ...
    public sealed class OpenGaussConnection : DbConnection, ICloneable, IComponent
    {
        #region Fields
        ...
        /// </summary>
        internal OpenGaussConnector? Connector { get; set; }

        /// <summary>
        /// The parsed connection string. Set only after the connection is opened.
        /// </summary>
        public OpenGaussConnectionStringBuilder Settings { get; private set; } = DefaultSettings;

        static readonly OpenGaussConnectionStringBuilder DefaultSettings = new();
        ...
        /// </summary>
        internal OpenGaussCommand? CachedCommand { get; set; }
        ...
        /// </summary>
        public static IOpenGaussTypeMapper GlobalTypeMapper => TypeMapping.GlobalTypeMapper.Instance;

        /// <summary>
        /// The connection-specific type mapper - all modifications affect this connection only,
        /// and are lost when it is closed.
        /// </summary>
        public IOpenGaussTypeMapper TypeMapper
        {
            get
            {
        ...
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(OpenGaussConnection));
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="OpenGaussConnection"/> class.
        /// </summary>
        public OpenGaussConnection()
            => GC.SuppressFinalize(this);

        /// <summary>
        /// Initializes a new instance of <see cref="OpenGaussConnection"/> with the given connection string.
        /// </summary>
        /// <param name="connectionString">The connection used to open the PostgreSQL database.</param>
        public OpenGaussConnection(string? connectionString) : this()
        ...
        /// </summary>
        /// <remarks>
        /// Do not invoke other methods and properties of the <see cref="OpenGaussConnection"/> object until the returned Task is complete.
        /// </remarks>
        ...
        void GetPoolAndSettings()
        {
            ...
            var settings = new OpenGaussConnectionStringBuilder(_connectionString);
            settings.Validate();
            Settings = settings;
            ...
            if (hostsSeparator is -1)
            {
                ...
                if (!OpenGaussConnectionStringBuilder.IsUnixSocket(settings.Host!, settings.Port, out _) &&
                    OpenGaussConnectionStringBuilder.TrySplitHostPort(settings.Host!.AsSpan(), out var newHost, out var newPort))
                {
            ...
            // ordering). Have OpenGaussConnectionStringBuilder produce a canonical string representation
            // and recheck.
            ...
            if (_pool == newPool)
            {
                ...
                OpenGaussEventSource.Log.PoolCreated(newPool);
            }
            else
                newPool.Dispose();
        ...
        internal Task Open(bool async, CancellationToken cancellationToken)
        {
            ...
            async Task OpenAsync(bool async, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    var timeout = new OpenGaussTimeout(connectionTimeout);
                    ...
                    timeout = new OpenGaussTimeout(connectionTimeout);
            ...
            async Task BootstrapMultiplexing(bool async, CancellationToken cancellationToken)
            {
                try
                {
                    var timeout = new OpenGaussTimeout(TimeSpan.FromSeconds(ConnectionTimeout));
        ...
        /// </p>
        /// <p>
        /// The <see cref="OpenGaussConnectionStringBuilder.Password"/> and <see cref="OpenGaussConnectionStringBuilder.Passfile"/> connection
            ...
            get => _fullState switch
                {
                    ConnectionState.Open => Connector == null
                        ...
                        : Connector.State switch
                        {
                            ...
                            ConnectorState.Closed      => throw new InvalidOperationException("Internal OpenGauss bug: connection is in state Open but connector is in state Closed"),
                            _ => throw new InvalidOperationException($"Internal OpenGauss bug: unexpected value {Connector.State} of enum {nameof(ConnectorState)}. Please file a bug.")
                        },
                    _ => _fullState
                };
            internal set
        ...
        /// <summary>
        /// Creates and returns a <see cref="OpenGaussCommand"/> object associated with the <see cref="OpenGaussConnection"/>.
        /// </summary>
        /// <returns>A <see cref="OpenGaussCommand"/> object.</returns>
        public new OpenGaussCommand CreateCommand()
        {
            CheckDisposed();
            ...
            return OpenGaussCommand.CreateCachedCommand(this);
        }
        ...
        public new OpenGaussBatch CreateBatch() => new(this);
#else
        /// <summary>
        /// Creates and returns a <see cref="OpenGaussBatch"/> object associated with the <see cref="OpenGaussConnection"/>.
        /// </summary>
        /// <returns>A <see cref="OpenGaussBatch"/> object.</returns>
        public OpenGaussBatch CreateBatch() => new(this);
#endif
        ...
        /// </summary>
        /// <returns>A <see cref="OpenGaussTransaction"/> object representing the new transaction.</returns>
        /// <remarks>
        /// Nested transactions are not supported.
        /// Transactions created by this method will have the <see cref="IsolationLevel.ReadCommitted"/> isolation level.
        /// </remarks>
        public new OpenGaussTransaction BeginTransaction()
        ...
        /// <returns>A <see cref="OpenGaussTransaction"/> object representing the new transaction.</returns>
        /// <remarks>Nested transactions are not supported.</remarks>
        public new OpenGaussTransaction BeginTransaction(IsolationLevel level)
            => BeginTransaction(level, async: false, CancellationToken.None).GetAwaiter().GetResult();

        async ValueTask<OpenGaussTransaction> BeginTransaction(IsolationLevel level, bool async, CancellationToken cancellationToken)
        {
            ...
            var connector = ConnectorBindingScope == ConnectorBindingScope.Transaction ?
                    Connector
                    : await StartBindingScope(ConnectorBindingScope.Transaction, OpenGaussTimeout.Infinite, async,
                        cancellationToken);
            ...
            try
            {
                ...
                connector.Transaction ??= new OpenGaussTransaction(connector);
        ...
        /// Transactions created by this method will have the <see cref="IsolationLevel.ReadCommitted"/> isolation level.
        /// </remarks>
        public new ValueTask<OpenGaussTransaction> BeginTransactionAsync(CancellationToken cancellationToken = default)
        ...
        /// Nested transactions are not supported.
        /// </remarks>
        public new ValueTask<OpenGaussTransaction> BeginTransactionAsync(IsolationLevel level, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
        ...
        {
            // Even though OpenGaussConnection isn't thread safe we'll make sure this part is.
        ...
        async Task CloseAsync(bool async)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        // We're already doing the same in the OpenGaussConnector.Reset for pooled connections
        ...
        /// <summary>
        /// Releases all resources used by the <see cref="OpenGaussConnection"/>.
        /// </summary>
        ...
        internal void OnNotice(PostgresNotice e)
        {
            try
            {
                Notice?.Invoke(this, new OpenGaussNoticeEventArgs(e));
            }
            catch (Exception ex)
            {
        ...
        internal void OnNotification(OpenGaussNotificationEventArgs e)
        {
            try
            {
        ...
        /// Ignored if <see cref="OpenGaussConnectionStringBuilder.SslMode"/> is set to <see cref="SslMode.Allow"/>,
        ...
        /// <returns>A <see cref="OpenGaussBinaryImporter"/> which can be used to write rows and columns</returns>
        /// <remarks>
        /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
        /// </remarks>
        public OpenGaussBinaryImporter BeginBinaryImport(string copyFromCommand)
        ...
        /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
        /// </remarks>
        public Task<OpenGaussBinaryImporter> BeginBinaryImportAsync(string copyFromCommand, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
                return BeginBinaryImport(copyFromCommand, async: true, cancellationToken);
        }

        async Task<OpenGaussBinaryImporter> BeginBinaryImport(string copyFromCommand, bool async, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var importer = new OpenGaussBinaryImporter(connector);
        ...
        /// <returns>A <see cref="OpenGaussBinaryExporter"/> which can be used to read rows and columns</returns>
        /// <remarks>
        /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
        /// </remarks>
        public OpenGaussBinaryExporter BeginBinaryExport(string copyToCommand)
        ...
        /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
        /// </remarks>
        public Task<OpenGaussBinaryExporter> BeginBinaryExportAsync(string copyToCommand, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
                return BeginBinaryExport(copyToCommand, async: true, cancellationToken);
        } 

        async Task<OpenGaussBinaryExporter> BeginBinaryExport(string copyToCommand, bool async, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var exporter = new OpenGaussBinaryExporter(connector);
        ...
        async Task<TextWriter> BeginTextImport(string copyFromCommand, bool async, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var copyStream = new OpenGaussRawCopyStream(connector);
                await copyStream.Init(copyFromCommand, async, cancellationToken);
                var writer = new OpenGaussCopyTextWriter(connector, copyStream);
        ...
        async Task<TextReader> BeginTextExport(string copyToCommand, bool async, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var reader = new OpenGaussCopyTextReader(connector, copyStream);
        ...
        /// <returns>A <see cref="OpenGaussRawCopyStream"/> that can be used to read or write raw binary data.</returns>
        /// <remarks>
        /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
        /// </remarks>
        public OpenGaussRawCopyStream BeginRawBinaryCopy(string copyCommand)
        ...
        /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
        /// </remarks>
        public Task<OpenGaussRawCopyStream> BeginRawBinaryCopyAsync(string copyCommand, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
                return BeginRawBinaryCopy(copyCommand, async: true, cancellationToken);
        }

        async Task<OpenGaussRawCopyStream> BeginRawBinaryCopy(string copyCommand, bool async, CancellationToken cancellationToken = default)
        {
            if (copyCommand == null)
            ...
            try
            {
                var stream = new OpenGaussRawCopyStream(connector);
        ...
        [Obsolete("Use OpenGaussConnection.TypeMapper.MapEnum() instead")]
        public void MapEnum<TEnum>(string? pgName = null, IOpenGaussNameTranslator? nameTranslator = null)
        ...
        /// </param>
        /// <typeparam name="TEnum">The .NET enum type to be mapped</typeparam>
        [Obsolete("Use OpenGaussConnection.GlobalTypeMapper.MapEnum() instead")]
        public static void MapEnumGlobally<TEnum>(string? pgName = null, IOpenGaussNameTranslator? nameTranslator = null)
            where TEnum : struct, Enum
        ...
        /// Defaults to <see cref="OpenGaussSnakeCaseNameTranslator"/>
        /// </param>
        [Obsolete("Use OpenGaussConnection.GlobalTypeMapper.UnmapEnum() instead")]
        public static void UnmapEnumGlobally<TEnum>(string? pgName = null, IOpenGaussNameTranslator? nameTranslator = null)
            where TEnum : struct, Enum
        ...
        [Obsolete("Use OpenGaussConnection.TypeMapper.MapComposite() instead")]
        [RequiresUnreferencedCode("Composite type mapping currently isn't trimming-safe.")]
        public void MapComposite<T>(string? pgName = null, IOpenGaussNameTranslator? nameTranslator = null) where T : new()
        ...
        /// </param>
        /// <typeparam name="T">The .NET type to be mapped</typeparam>
        [Obsolete("Use OpenGaussConnection.GlobalTypeMapper.MapComposite() instead")]
        [RequiresUnreferencedCode("Composite type mapping currently isn't trimming-safe.")]
        public static void MapCompositeGlobally<T>(string? pgName = null, IOpenGaussNameTranslator? nameTranslator = null) where T : new()
        ...
        /// Defaults to <see cref="OpenGaussSnakeCaseNameTranslator"/>
        /// </param>
        [Obsolete("Use OpenGaussConnection.GlobalTypeMapper.UnmapComposite() instead")]
        [RequiresUnreferencedCode("Composite type mapping currently isn't trimming-safe.")]
        public static void UnmapCompositeGlobally<T>(string pgName, IOpenGaussNameTranslator? nameTranslator = null) where T : new()
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(typeof(OpenGaussConnection).Name);
        }
        ...
        /// </summary>
        internal bool TryGetBoundConnector([NotNullWhen(true)] out OpenGaussConnector? connector)
        {
            if (ConnectorBindingScope == ConnectorBindingScope.None)
            {
        ...
        /// </summary>
        internal ValueTask<OpenGaussConnector> StartBindingScope(
            ConnectorBindingScope scope, OpenGaussTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            ...
            if (ConnectorBindingScope != ConnectorBindingScope.None)
            {
                ...
                return new ValueTask<OpenGaussConnector>(Connector);
            }

            return StartBindingScopeAsync();

            async ValueTask<OpenGaussConnector> StartBindingScopeAsync()
            {
                try
                {
                    Debug.Assert(Settings.Multiplexing);
        ...
        internal OpenGaussConnector StartBindingScope(ConnectorBindingScope scope)
            => StartBindingScope(scope, OpenGaussTimeout.Infinite, async: false, CancellationToken.None)
                .GetAwaiter().GetResult();

        internal EndScopeDisposable StartTemporaryBindingScope(out OpenGaussConnector connector)
        {
            connector = StartBindingScope(ConnectorBindingScope.Temporary);
            return new EndScopeDisposable(this);
        }

        internal T CheckOpenAndRunInTemporaryScope<T>(Func<OpenGaussConnector, T> func)
        {
            CheckOpen();
        ...
        public override DataTable GetSchema(string? collectionName, string?[]? restrictions)
            => OpenGaussSchema.GetSchema(this, collectionName, restrictions, async: false).GetAwaiter().GetResult();
        ...
        {
            using (NoSynchronizationContextScope.Enter())
                return OpenGaussSchema.GetSchema(this, collectionName, restrictions, async: true, cancellationToken);
        }
        ...
        {
            CheckDisposed();
            var conn = new OpenGaussConnection(_connectionString) {
        ...
        /// database or pooling)
        /// </summary>
        public OpenGaussConnection CloneWith(string connectionString)
        {
            CheckDisposed();
            ...
            return new OpenGaussConnection(csb.ToString()) {
                ProvideClientCertificatesCallback = ProvideClientCertificatesCallback,
                UserCertificateValidationCallback = UserCertificateValidationCallback,
                ProvidePasswordCallback = ProvidePasswordCallback,
            };
        ...
        /// DB provider factory.
        /// </summary>
        protected override DbProviderFactory DbProviderFactory => OpenGaussFactory.Instance;
        ...
        /// </summary>
        public static void ClearPool(OpenGaussConnection connection) => PoolManager.Clear(connection._connectionString);
        ...
        /// <summary>
        /// This event is unsupported by OpenGauss. Use <see cref="DbConnection.StateChange"/> instead.
        /// </summary>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public new event EventHandler? Disposed
        {
            add => throw new NotSupportedException("The Disposed event isn't supported by OpenGauss. Use DbConnection.StateChange instead.");
            remove => throw new NotSupportedException("The Disposed event isn't supported by OpenGauss. Use DbConnection.StateChange instead.");
        }
    ...
    {
        readonly OpenGaussConnection _connection;
        public EndScopeDisposable(OpenGaussConnection connection) => _connection = connection;
    ...
    /// <summary>
    /// Represents a method that handles the <see cref="OpenGaussConnection.Notice"/> event.
    /// </summary>
    /// <param name="sender">The source of the event.</param>
    /// <param name="e">A <see cref="OpenGaussNoticeEventArgs"/> that contains the notice information (e.g. message, severity...).</param>
    public delegate void NoticeEventHandler(object sender, OpenGaussNoticeEventArgs e);

    /// <summary>
    /// Represents a method that handles the <see cref="OpenGaussConnection.Notification"/> event.
    /// </summary>
    /// <param name="sender">The source of the event.</param>
    /// <param name="e">A <see cref="OpenGaussNotificationEventArgs"/> that contains the notification payload.</param>
    public delegate void NotificationEventHandler(object sender, OpenGaussNotificationEventArgs e);
    ...
    [RequiresPreviewFeatures("Physical open callback is an experimental API, and its exact shape may change in the future")]
    public delegate void PhysicalOpenCallback(OpenGaussConnector connection);
    ...
    /// <param name="connection">Physical connection to the database</param>
    [RequiresPreviewFeatures("Physical open callback is an experimental API, and its exact shape may change in the future")]
    public delegate Task PhysicalOpenAsyncCallback(OpenGaussConnector connection);
