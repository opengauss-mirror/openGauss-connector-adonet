...
namespace OpenGauss.NET.Schema
{
    class DbColumnSchemaGenerator
    {
        ...
        internal DbColumnSchemaGenerator(OpenGaussConnection connection, RowDescriptionMessage rowDescription, bool fetchAdditionalInfo)
        {
...
ORDER BY attnum";
        ...
        internal async Task<ReadOnlyCollection<OpenGaussDbColumn>> GetColumnSchema(bool async, CancellationToken cancellationToken = default)
        {
            ...
            var result = new List<OpenGaussDbColumn?>(numFields);
            ...
            if (_fetchAdditionalInfo)
            {
				...
				if (columnFieldFilter != string.Empty)
				{
					...
					using var connection = (OpenGaussConnection)((ICloneable)_connection).Clone();
	
					await connection.Open(async, cancellationToken);

                    using var cmd = new OpenGaussCommand(query, connection);
            ...
            if (populatedColumns != numFields)
                throw new OpenGaussException("Could not load all columns for the resultset");

            return result.AsReadOnly()!;
        }

        OpenGaussDbColumn LoadColumnDefinition(OpenGaussDataReader reader, OpenGaussDatabaseInfo databaseInfo, bool oldQueryMode)
        {
            ...
            var column = new OpenGaussDbColumn
            {
        ...
        OpenGaussDbColumn SetUpNonColumnField(FieldDescription field)
        {
        ...
        /// </summary>
        void ColumnPostConfig(OpenGaussDbColumn column, int typeModifier)
        {
            var typeMapper = _connection.Connector!.TypeMapper;

            column.OpenGaussDbType = typeMapper.GetTypeInfoByOid(column.TypeOID).opengaussDbType;
