...
namespace OpenGauss.NET
{
    class SqlQueryParser
    {
        ...
        /// <summary>
        /// <p>
        /// Receives a user SQL query as passed in by the user in <see cref="OpenGaussCommand.CommandText"/> or
        /// <see cref="OpenGaussBatchCommand.CommandText"/>, and rewrites it for PostgreSQL compatibility.
        /// </p>
        /// <p>
        ...
        /// </p>
        /// </summary>
        /// <param name="command">The user-facing <see cref="OpenGaussCommand"/> being executed.</param>
        ...
        /// </p>
        /// </summary>
        /// <param name="batchCommand"> The user-facing <see cref="OpenGaussBatchCommand"/> being executed.</param>
        /// <param name="standardConformingStrings">Whether PostgreSQL standards-conforming are used.</param>
        /// <param name="deriveParameters">
        /// A bool indicating whether parameters contains a list of preconfigured parameters or an empty list to be filled with derived
        /// parameters.
        /// </param>
        internal void ParseRawQuery(
            OpenGaussBatchCommand? batchCommand,
            bool standardConformingStrings = true,
            bool deriveParameters = false)
            ...
            bool deriveParameters = false)
        {
            ...
            OpenGaussParameterCollection parameters;
            List<OpenGaussBatchCommand>? batchCommands;
                ...
                if (currCharOfs >= end || !IsParamNameChar(ch = sql[currCharOfs]))
                {
                    ...
                    if (!_paramIndexMap.TryGetValue(paramName, out var index))
                    {
                        ...
                        if (!parameters.TryGetValue(paramName, out var parameter))
                        {
                            if (deriveParameters)
                            {
                                parameter = new OpenGaussParameter { ParameterName = paramName };
            ...
            while (currCharOfs < end)
            {
                ...
                if (command is null)
                {
                    throw new NotSupportedException(
                        $"Specifying multiple SQL statements in a single {nameof(OpenGaussBatchCommand)} isn't supported, " +
        ...
        Finish:
            ...
            void MoveToNextBatchCommand()
            {
                ...
                {
                    batchCommand = new OpenGaussBatchCommand();
