...
{
    static class OpenGaussActivitySource
    {
        static readonly ActivitySource Source;

        static OpenGaussActivitySource()
        {
            var assembly = typeof(OpenGaussActivitySource).Assembly;
            var version = assembly.GetCustomAttribute<AssemblyFileVersionAttribute>()?.Version ?? "0.0.0";
            Source = new("OpenGauss", version);
        }

        internal static bool IsEnabled => Source.HasListeners();

        internal static Activity? CommandStart(OpenGaussConnector connector, string sql)
        {
