...
namespace OpenGauss.NET
{
    class PreparedStatementManager
    {
        ...
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(PreparedStatementManager));

        internal const int CandidateCount = 100;

        internal PreparedStatementManager(OpenGaussConnector connector)
        {
        ...
        internal PreparedStatement? GetOrAddExplicit(OpenGaussBatchCommand batchCommand)
        {
        ...
        internal PreparedStatement? TryGetAutoPrepared(OpenGaussBatchCommand batchCommand)
        {
            var sql = batchCommand.FinalCommandText!;
            if (!BySql.TryGetValue(sql, out var pStatement))
            {
