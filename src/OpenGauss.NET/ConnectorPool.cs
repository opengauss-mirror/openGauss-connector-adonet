...
namespace OpenGauss.NET
{
    class ConnectorPool : ConnectorSource
    {
        ...
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(ConnectorPool));
        ...
        /// </summary>
        private protected readonly OpenGaussConnector?[] Connectors;
        ...
        /// </summary>
        readonly ChannelReader<OpenGaussConnector?> _idleConnectorReader;
        internal ChannelWriter<OpenGaussConnector?> IdleConnectorWriter { get; }

        /// <summary>
        /// Incremented every time this pool is cleared via <see cref="OpenGaussConnection.ClearPool"/> or
        /// <see cref="OpenGaussConnection.ClearAllPools"/>. Allows us to identify connections which were
        ...
        internal ConnectorPool(OpenGaussConnectionStringBuilder settings, string connString, MultiHostConnectorPool? parentPool = null)
            : base(settings, connString)
        {
            ...
            var idleChannel = Channel.CreateUnbounded<OpenGaussConnector?>();
            ...
            Connectors = new OpenGaussConnector[_max];
        }

        internal sealed override ValueTask<OpenGaussConnector> Get(
            OpenGaussConnection conn, OpenGaussTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            return TryGetIdleConnector(out var connector)
                ? new ValueTask<OpenGaussConnector>(connector)
                : RentAsync(conn, timeout, async, cancellationToken);

            async ValueTask<OpenGaussConnector> RentAsync(
                OpenGaussConnection conn, OpenGaussTimeout timeout, bool async, CancellationToken cancellationToken)
            {
                ...
                while (true)
                {
                    ...
                    {
                        throw new OpenGaussException("The connection pool has been shut down.");
                    }
        ...
        internal sealed override bool TryGetIdleConnector([NotNullWhen(true)] out OpenGaussConnector? connector)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        bool CheckIdleConnector([NotNullWhen(true)] OpenGaussConnector? connector)
        {
        ...
        internal sealed override async ValueTask<OpenGaussConnector?> OpenNewConnector(
            OpenGaussConnection conn, OpenGaussTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            ...
            for (var numConnectors = _numConnectors; numConnectors < _max; numConnectors = _numConnectors)
            {
                ...
                try
                {
                    ...
                    var connector = new OpenGaussConnector(this, conn) { ClearCounter = _clearCounter };
                    ...
                    if (i == _max)
                        throw new OpenGaussException($"Could not find free slot in {Connectors} when opening. Please report a bug.");
        ...
        internal sealed override void Return(OpenGaussConnector connector)
        {
        ...
        void CloseConnector(OpenGaussConnector connector)
        {
            try
            {
                connector.Close();
            ...
            if (i == _max)
                throw new OpenGaussException($"Could not find free slot in {Connectors} when closing. Please report a bug.");
