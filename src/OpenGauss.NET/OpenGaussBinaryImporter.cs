...
namespace OpenGauss.NET
{
    ...
    /// a PostgreSQL table. Initiated by <see cref="OpenGaussConnection.BeginBinaryImport(string)"/>
    /// </summary>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public sealed class OpenGaussBinaryImporter : ICancelable
    {
        #region Fields and Properties

        OpenGaussConnector _connector;
        OpenGaussWriteBuffer _buf;
        ...
        OpenGaussParameter?[] _params;

        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(OpenGaussBinaryImporter));
        ...
        internal OpenGaussBinaryImporter(OpenGaussConnector connector)
        {
        ...
        internal async Task Init(string copyFromCommand, bool async, CancellationToken cancellationToken = default)
        {
            ...
            _params = new OpenGaussParameter[NumColumns];
            _buf.StartCopyMode();
            WriteHeader();
        }
        ...
        void WriteHeader()
        {
            _buf.WriteBytes(OpenGaussRawCopyStream.BinarySignature, 0, OpenGaussRawCopyStream.BinarySignature.Length);
        ...
        /// corruption will occur. If in doubt, use <see cref="Write{T}(T, OpenGaussDbType)"/> to manually
        /// specify the type.
        /// </typeparam>
        ...
        /// <summary>
        /// Writes a single column in the current row as type <paramref name="opengaussDbType"/>.
        /// </summary>
        /// <param name="value">The value to be written</param>
        /// <param name="opengaussDbType">
        ...
        /// <paramref name="opengaussDbType"/> must be specified as <see cref="OpenGaussDbType.Jsonb"/>.
        /// </param>
        /// <typeparam name="T">The .NET type of the column to be written.</typeparam>
        public void Write<T>([AllowNull] T value, OpenGaussDbType opengaussDbType) =>
            Write(value, opengaussDbType, false).GetAwaiter().GetResult();
        ...
        /// </param>
        /// <typeparam name="T">The .NET type of the column to be written.</typeparam>
        public Task WriteAsync<T>([AllowNull] T value, OpenGaussDbType opengaussDbType, CancellationToken cancellationToken = default)
        {
            if (cancellationToken.IsCancellationRequested)
                return Task.FromCanceled(cancellationToken);
            using (NoSynchronizationContextScope.Enter())
                return Write(value, opengaussDbType, true, cancellationToken);
        }

        Task Write<T>([AllowNull] T value, OpenGaussDbType opengaussDbType, bool async, CancellationToken cancellationToken = default)
        {
            CheckColumnIndex();
            ...
            if (p == null)
            {
                ...
                _params[_column] = p = typeof(T) == typeof(object)
                    ? new OpenGaussParameter()
                    : new OpenGaussParameter<T>();
                p.OpenGaussDbType = opengaussDbType;
            }

            if (opengaussDbType != p.OpenGaussDbType)
                throw new InvalidOperationException($"Can't change {nameof(p.OpenGaussDbType)} from {p.OpenGaussDbType} to {opengaussDbType}");
        ...
        async Task Write<T>([AllowNull] T value, OpenGaussParameter param, bool async, CancellationToken cancellationToken = default)
        {
            CheckReady();
            if (_column == -1)
            ...
            {
                if (param is not OpenGaussParameter<T> typedParam)
                {
                    _params[_column] = typedParam = new OpenGaussParameter<T>();
                    typedParam.OpenGaussDbType = param.OpenGaussDbType;
                    param = typedParam;
                }
        ...
        async Task Cancel(bool async, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                throw _connector.Break(
                    new OpenGaussException("Expected ErrorResponse when cancelling COPY but got: " + msg.Code));
            }
