...
namespace OpenGauss.NET
{
    ...
    /// </summary>
    public sealed class OpenGaussTransaction : DbTransaction
    {
        #region Fields and Properties

        /// <summary>
        /// Specifies the <see cref="OpenGaussConnection"/> object associated with the transaction.
        /// </summary>
        /// <value>The <see cref="OpenGaussConnection"/> object associated with the transaction.</value>
        ...
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(OpenGaussTransaction));
        ...
        internal OpenGaussTransaction(OpenGaussConnector connector)
        ...
        void CheckReady()
        {
            CheckDisposed();
            if (IsCompleted)
                throw new InvalidOperationException("This OpenGaussTransaction has completed; it is no longer usable.");
        }

        void CheckDisposed()
        {
            if (IsDisposed)
                throw new ObjectDisposedException(typeof(OpenGaussTransaction).Name, _disposeReason);
        }
