...
namespace OpenGauss.NET
{
    ...
    /// </summary>
    static class OpenGaussSchema
    {
        public static Task<DataTable> GetSchema(OpenGaussConnection conn, string? collectionName, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
            ...
            return collectionName.ToUpperInvariant() switch
            {
                ...
                // custom collections for opengauss
        ...
        static OpenGaussCommand BuildCommand(OpenGaussConnection conn, StringBuilder query, string?[]? restrictions, params string[]? names)
            => BuildCommand(conn, query, restrictions, true, names);

        static OpenGaussCommand BuildCommand(OpenGaussConnection conn, StringBuilder query, string?[]? restrictions, bool addWhere, params string[]? names)
        {
            ...
            if (restrictions != null && names != null)
            {
                for (var i = 0; i < restrictions.Length && i < names.Length; ++i)
                {
                    if (restrictions[i] is string restriction && restriction.Length != 0)
                    {
                        ...
                        command.Parameters.Add(new OpenGaussParameter(paramName, restriction));
                    }
                }
            }
        ...
        static async Task<DataTable> GetDatabases(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
            ...
            using var adapter = new OpenGaussDataAdapter(command);
            await adapter.Fill(databases, async, cancellationToken);

            return databases;
        }

        static async Task<DataTable> GetSchemata(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
...
SELECT * FROM (
    ...
    FROM
        ...
        static async Task<DataTable> GetTables(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
...
WHERE
    ...
    table_schema NOT IN ('pg_catalog', 'information_schema')");
        ...
        static async Task<DataTable> GetColumns(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
...
FROM information_schema.columns");
        ...
        static async Task<DataTable> GetViews(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
...
WHERE table_schema NOT IN ('pg_catalog', 'information_schema')");
        ...
        static async Task<DataTable> GetUsers(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
        ...
        static async Task<DataTable> GetIndexes(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
...
WHERE
    ...
    t.relkind = 'r'");
        ...
        static async Task<DataTable> GetIndexColumns(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
...
WHERE
    ...
    t.relkind = 'r'");
        ...
        static async Task<DataTable> GetConstraints(OpenGaussConnection conn, string?[]? restrictions, string? constraintType, bool async, CancellationToken cancellationToken = default)
        {
...
FROM
    ...
    JOIN (
        ...
        static async Task<DataTable> GetConstraintColumns(OpenGaussConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
        {
...
FROM pg_constraint c
    ...
    AND n.nspname NOT IN ('pg_catalog', 'pg_toast')");
        ...
        static DataTable GetDataSourceInformation(OpenGaussConnection conn)
        {
            ...
            row["DataSourceProductName"] = "OpenGauss";
            ...
            if (OpenGaussCommand.EnableSqlRewriting)
            {
        ...
        static DataTable GetDataTypes(OpenGaussConnection conn)
        {
                ...
                .Concat(connector.DatabaseInfo.CompositeTypes))
            {
                ...
                if (mapping.OpenGaussDbType.HasValue)
                    row["ProviderDbType"] = (int)mapping.OpenGaussDbType.Value;
            }
            ...
            foreach (var arrayType in connector.DatabaseInfo.ArrayTypes)
            {
                ...
                if (elementMapping.OpenGaussDbType.HasValue)
                    row["ProviderDbType"] = (int)(elementMapping.OpenGaussDbType.Value | OpenGaussDbType.Array);
            }
            ...
            foreach (var rangeType in connector.DatabaseInfo.RangeTypes)
            {
                ...
                if (subtypeMapping.ClrTypes.Length > 0)
                    row["DataType"] = typeof(OpenGaussRange<>).MakeGenericType(subtypeMapping.ClrTypes[0]).FullName;
                if (subtypeMapping.OpenGaussDbType.HasValue)
                    row["ProviderDbType"] = (int)(subtypeMapping.OpenGaussDbType.Value | OpenGaussDbType.Range);
            }
            ...
            foreach (var domainType in connector.DatabaseInfo.DomainTypes)
            {
                ...
                if (baseMapping.OpenGaussDbType.HasValue)
                    row["ProviderDbType"] = (int)baseMapping.OpenGaussDbType.Value;
            }
