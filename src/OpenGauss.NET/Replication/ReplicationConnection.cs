...
namespace OpenGauss.NET.Replication
{
    ...
    public abstract class ReplicationConnection : IAsyncDisposable
    {
        ...
        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(ReplicationConnection));
        readonly OpenGaussConnection _opengaussConnection;
        ...
        {
            _opengaussConnection = new OpenGaussConnection();
        ...
        /// <see cref="OpenGaussConnectionStringBuilder.Pooling"/>, <see cref="OpenGaussConnectionStringBuilder.Enlist"/>,
        /// <see cref="OpenGaussConnectionStringBuilder.Multiplexing" /> and <see cref="OpenGaussConnectionStringBuilder.KeepAlive"/>
        ...
        public string ConnectionString {
            ...
            set
            {
                ...
                _opengaussConnection.ConnectionString = new OpenGaussConnectionStringBuilder(value)
                {
                    Pooling = false,
                    Enlist = false,
                    Multiplexing = false,
        ...
        /// </summary>
        public OpenGaussLogSequenceNumber LastReceivedLsn
        {
            get => (OpenGaussLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastReceivedLsn));
        ...
        /// </summary>
        public OpenGaussLogSequenceNumber LastFlushedLsn
        {
            get => (OpenGaussLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastFlushedLsn));
        ...
        /// </summary>
        public OpenGaussLogSequenceNumber LastAppliedLsn
        {
            get => (OpenGaussLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastAppliedLsn));
        ...
        /// </remarks>
        /// </summary>
        public Version PostgreSqlVersion => _opengaussConnection.PostgreSqlVersion;
        ...
        /// </remarks>
        /// </summary>
        public string ServerVersion => _opengaussConnection.ServerVersion;

        internal OpenGaussConnector Connector
            => _opengaussConnection.Connector ??
        ...
        {
            get => _commandTimeout ?? (_opengaussConnection.CommandTimeout > 0
                ? TimeSpan.FromSeconds(_opengaussConnection.CommandTimeout)
        ...
        /// </summary>
        public Encoding Encoding => _opengaussConnection.Connector?.TextEncoding ?? throw new InvalidOperationException($"The {nameof(Encoding)} property can only be used when there is an active connection");

        /// <summary>
        /// Process id of backend server.
        /// This can only be called when there is an active connection.
        /// </summary>
        public int ProcessID => _opengaussConnection.Connector?.BackendProcessId ?? throw new InvalidOperationException($"The {nameof(ProcessID)} property can only be used when there is an active connection");
        ...
        public async Task Open(CancellationToken cancellationToken = default)
        {
            ...
            await _opengaussConnection.OpenAsync(cancellationToken)
                .ConfigureAwait(false);

            // PG versions before 10 ignore cancellations during replication
            _pgCancellationSupported = _opengaussConnection.PostgreSqlVersion.IsGreaterOrEqual(10, 0);
        ...
        public ValueTask DisposeAsync()
        {
            ...
            async ValueTask DisposeAsyncCore()
            {
                ...
                if (_opengaussConnection.Connector?.State == ConnectorState.Replication)
                {
                ...
                try
                {
                    await _opengaussConnection.Close(async: true);
                }
                catch
                {
                    // Dispose
        ...
        public Task<ReplicationSystemIdentification> IdentifySystem(CancellationToken cancellationToken = default)
        {
            ...
            async Task<ReplicationSystemIdentification> IdentifySystemInternal(CancellationToken cancellationToken)
            {
                ...
                return new ReplicationSystemIdentification(
                    (string)row[0], (uint)row[1], OpenGaussLogSequenceNumber.Parse((string)row[2]), (string)row[3]);
            }
        }
            ...
            string command, bool temporarySlot, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                await buf.EnsureAsync(len);
                var consistentPoint   = OpenGaussLogSequenceNumber.Parse(buf.ReadString(len));
            ...
            CancellationToken cancellationToken)
        {
            ...
            var connector = _opengaussConnection.Connector!;
            ...
            try
            {
                ...
                var columnStream = new OpenGaussReadBuffer.ColumnStream(connector, startCancellableOperations: false);
                ...
                while (true)
                {
                    ...
                    case 'w': // XLogData
                    {
                        ...
                        _cachedXLogDataMessage.Populate(new OpenGaussLogSequenceNumber(startLsn), new OpenGaussLogSequenceNumber(endLsn),
                    ...
                    case 'k': // Primary keepalive message
                    {
                        ...
                        if (Log.IsEnabled(OpenGaussLogLevel.Trace))
                        {
                            var endLsn = new OpenGaussLogSequenceNumber(end);
                    ...
                    default:
                        throw Connector.Break(new OpenGaussException($"Unknown replication message code '{code}'"));
                    }
                }
            }
            finally
        ...
        public void SetReplicationStatus(OpenGaussLogSequenceNumber lastAppliedAndFlushedLsn)
        {
        ...
        public Task SendStatusUpdate(CancellationToken cancellationToken = default)
        {
            ...
            async Task SendStatusUpdateInternal(CancellationToken cancellationToken)
            {
                ...
                // However, we generally aren't safe for this in OpenGauss, leaving as-is for now.
                if (Connector.State != ConnectorState.Replication)
            ...
            try
            {
                ...
                if (Log.IsEnabled(OpenGaussLogLevel.Trace))
                {
                    Log.Trace("Feedback message sent with " +
                              $"{nameof(LastReceivedLsn)} of {new OpenGaussLogSequenceNumber(unchecked((ulong)lastReceivedLsn))}, " +
                              $"{nameof(LastFlushedLsn)} of {new OpenGaussLogSequenceNumber(unchecked((ulong)lastFlushedLsn))}, " +
                              $"{nameof(LastAppliedLsn)} of {new OpenGaussLogSequenceNumber(unchecked((ulong)lastAppliedLsn))} " +
        ...
        async void TimerRequestFeedback(object? obj)
        {
            ...
            {
                Log.Error("An exception occurred while requesting streaming replication feedback from the server.", e, _opengaussConnection?.Connector?.Id ?? 0);
            }
        }
        ...
        async void TimerSendFeedback(object? obj)
        {
            ...
            {
                Log.Error("An exception occurred while sending streaming replication feedback to the server.", e, _opengaussConnection?.Connector?.Id ?? 0);
            }
        }
        ...
        async Task<object[]> ReadSingleRow(string command, CancellationToken cancellationToken = default)
        {
            ...
            for (var i = 0; i < results.Length; i++)
            {
                ...
                case "bytea":
                    ...
                    {
                        throw Connector.Break(
                            new OpenGaussException($"Could not parse data as bytea in field {field.Name}", e));
                    }

                    continue;
                default:

                    throw Connector.Break(new OpenGaussException(
