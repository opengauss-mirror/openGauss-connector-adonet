...
namespace OpenGauss.NET
{
    ...
    /// </summary>
    /// <remarks>
    /// Note that a connection may be closed before its TransactionScope completes. In this case we close the OpenGaussConnection
    ...
    class VolatileResourceManager : ISinglePhaseNotification
    {
        ...
        OpenGaussTransaction _localTx = null!;
        string? _preparedTxName;
        bool IsPrepared => _preparedTxName != null;
        bool _isDisposed;

        static readonly OpenGaussLogger Log = OpenGaussLogManager.CreateLogger(nameof(VolatileResourceManager));

        const int MaximumRollbackAttempts = 20;

        internal VolatileResourceManager(OpenGaussConnection connection, Transaction transaction)
        {
        ...
        public void Commit(Enlistment enlistment)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    using var conn2 = (OpenGaussConnection)((ICloneable)_connector.Connection).Clone();
                    conn2.Open();
        ...
        void RollbackLocal()
        {
            ...
            while (true)
            {
                ...
                catch (OpenGaussOperationInProgressException)
                {
