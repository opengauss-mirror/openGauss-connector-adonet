...
namespace OpenGauss.NET.Internal
{
    ...
    /// </summary>
    public abstract class OpenGaussDatabaseInfo
    {
        #region Fields

        internal static readonly ConcurrentDictionary<OpenGaussDatabaseInfoCacheKey, OpenGaussDatabaseInfo> Cache
            = new();

        static volatile IOpenGaussDatabaseInfoFactory[] Factories = new IOpenGaussDatabaseInfoFactory[]
        {
        ...
        /// Exposed via <see cref="OpenGaussConnection.PostgreSqlVersion"/>.
        /// </summary>
        ...
        /// <summary>
        /// Initializes the instance of <see cref="OpenGaussDatabaseInfo"/>.
        /// </summary>
        protected OpenGaussDatabaseInfo(string host, int port, string databaseName, Version version)
        ...
        /// Initializes the instance of <see cref="OpenGaussDatabaseInfo"/>.
        /// </summary>
        protected OpenGaussDatabaseInfo(string host, int port, string databaseName, Version version, string serverVersion)
        {
            Host = host;
            Port = port;
            Name = databaseName;
        ...
        private protected OpenGaussDatabaseInfo(string host, int port, string databaseName, string serverVersion)
        {
            Host = host;
            Port = port;
            Name = databaseName;
        ...
        /// </summary>
        public static void RegisterFactory(IOpenGaussDatabaseInfoFactory factory)
        {
            if (factory == null)
                throw new ArgumentNullException(nameof(factory));

            var factories = new IOpenGaussDatabaseInfoFactory[Factories.Length + 1];
            factories[0] = factory;
        ...
        internal static async Task<OpenGaussDatabaseInfo> Load(OpenGaussConnector conn, OpenGaussTimeout timeout, bool async)
        {
            ...
            throw new OpenGaussException("No DatabaseInfoFactory could be found for this connection");
        }

        // For tests
        internal static void ResetFactories()
        {
            Factories = new IOpenGaussDatabaseInfoFactory[]
            {
                new PostgresMinimalDatabaseInfoFactory(),
                new PostgresDatabaseInfoFactory()
            };
