...
namespace OpenGauss.NET.Internal.TypeHandlers.DateTimeHandlers
{
    ...
    /// </remarks>
    public partial class TimestampTzHandler : OpenGaussSimpleTypeHandlerWithPsv<DateTime, OpenGaussDateTime>,
        IOpenGaussSimpleTypeHandler<DateTimeOffset>, IOpenGaussSimpleTypeHandler<long>
    {
        /// <summary>
        ...
        {
            var ts = ReadOpenGaussDateTime(buf, len, fieldDescription);

            if (!ts.IsFinite)
                return ts;

            var opengaussDateTime = new OpenGaussDateTime(ts.Date, ts.Time, DateTimeKind.Utc);
            return LegacyTimestampBehavior ? opengaussDateTime.ToLocalTime() : opengaussDateTime;
        }
        ...
        public override int ValidateAndGetLength(OpenGaussDateTime value, OpenGaussParameter? parameter)
            => value.Kind == DateTimeKind.Utc ||
               value == OpenGaussDateTime.Infinity ||
               value == OpenGaussDateTime.NegativeInfinity ||
               LegacyTimestampBehavior
                ? 8
                : throw new InvalidCastException(
                    $"Cannot write DateTime with Kind={value.Kind} to PostgreSQL type 'timestamp with time zone', only UTC is supported. " +
        ...
        /// <inheritdoc />
        public int ValidateAndGetLength(DateTimeOffset value, OpenGaussParameter? parameter)
        ...
        public override void Write(DateTime value, OpenGaussWriteBuffer buf, OpenGaussParameter? parameter)
        {
            if (LegacyTimestampBehavior)
            {
                ...
                case DateTimeKind.Local:
                    ...
                    break;
                default:
                    throw new InvalidOperationException($"Internal OpenGauss bug: unexpected value {value.Kind} of enum {nameof(DateTimeKind)}. Please file a bug.");
                }
            }
            else
        ...
        /// <inheritdoc />
        public void Write(DateTimeOffset value, OpenGaussWriteBuffer buf, OpenGaussParameter? parameter)
        {
            if (LegacyTimestampBehavior)
                value = value.ToUniversalTime();
