...
namespace OpenGauss.NET.Internal.TypeHandlers.DateTimeHandlers
{
    static class DateTimeUtils
    {
        ...
        internal static DateTime ReadDateTime(OpenGaussReadBuffer buf, DateTimeKind kind)
        {
            try
            {
...
#pragma warning disable 618 // OpenGaussDateTime is obsolete, remove in 7.0
        internal static OpenGaussDateTime ReadOpenGaussDateTime(OpenGaussReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            var value = buf.ReadInt64();
            if (value == long.MaxValue)
                return OpenGaussDateTime.Infinity;
            if (value == long.MinValue)
                return OpenGaussDateTime.NegativeInfinity;
            if (value >= 0)
            {
                ...
                return new OpenGaussDateTime(new OpenGaussDate(date), new TimeSpan(time));
            }
            else
            {
                value = -value;
...
#pragma warning restore 618
        ...
        internal static void WriteTimestamp(DateTime value, OpenGaussWriteBuffer buf)
        {
...
#pragma warning disable 618 // OpenGaussDateTime is obsolete, remove in 7.0
        internal static void WriteTimestamp(OpenGaussDateTime value, OpenGaussWriteBuffer buf)
        {
            if (value.IsInfinity)
            {
                buf.WriteInt64(long.MaxValue);
            ...
            if (value >= new OpenGaussDateTime(2000, 1, 1, 0, 0, 0))
            {
