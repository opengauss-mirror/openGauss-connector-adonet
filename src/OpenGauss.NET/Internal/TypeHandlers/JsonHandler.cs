...
namespace OpenGauss.NET.Internal.TypeHandlers
{
    ...
    /// </remarks>
    public class JsonHandler : OpenGaussTypeHandler<string>, ITextReaderHandler
    {
        ...
        /// <inheritdoc />
        protected override async Task WriteWithLengthCustom<TAny>([DisallowNull] TAny value, OpenGaussWriteBuffer buf, OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
        ...
        /// <inheritdoc />
        public override async Task WriteObjectWithLength(object? value, OpenGaussWriteBuffer buf, OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
        ...
        /// <inheritdoc />
        protected internal override async ValueTask<T> ReadCustom<T>(OpenGaussReadBuffer buf, int byteLen, bool async, FieldDescription? fieldDescription = null)
        {
            if (_isJsonb)
            {
        ...
        public TextReader GetTextReader(Stream stream)
        {
            if (_isJsonb)
            {
                ...
                if (version != JsonbProtocolVersion)
                    throw new OpenGaussException($"Don't know how to decode jsonb with wire format {version}, your connection is now broken");
            }
