...
namespace OpenGauss.NET.Internal.TypeHandlers
{
    ...
    class UnknownTypeHandler : TextHandler
    {
        ...
        internal UnknownTypeHandler(OpenGaussConnector connector)
            ...
            if (fieldDescription.IsBinaryFormat)
            {
                ...
                throw new NotSupportedException(
                    _connector.TypeMapper.DatabaseInfo.ByOID.TryGetValue(fieldDescription.TypeOID, out var pgType)
                        ? $"The field '{fieldDescription.Name}' has type '{pgType.DisplayName}', which is currently unknown to OpenGauss. You can retrieve it as a string by marking it as unknown, please see the FAQ."
                        : $"The field '{fieldDescription.Name}' has a type currently unknown to OpenGauss (OID {fieldDescription.TypeOID}). You can retrieve it as a string by marking it as unknown, please see the FAQ."
                );
            }
        ...
        protected internal override int ValidateAndGetLengthCustom<TAny>(
            [DisallowNull] TAny value, ref OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter)
        ...
        public override Task WriteObjectWithLength(object? value, OpenGaussWriteBuffer buf, OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
            ...
            async Task WriteWithLengthLong(object value, string convertedValue, OpenGaussWriteBuffer buf, OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter, bool async, CancellationToken cancellationToken)
            {
