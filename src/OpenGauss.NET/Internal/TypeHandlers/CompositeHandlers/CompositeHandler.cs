...
[module: UnconditionalSuppressMessage(
    ...
    "IL2046", Scope = "type", Target = "OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2080", Scope = "type", Target = "OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2026", Scope = "type", Target = "OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2090", Scope = "type", Target = "OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2087", Scope = "type", Target = "OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2055", Scope = "type", Target = "OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2077", Scope = "type", Target = "OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]

#endregion

namespace OpenGauss.NET.Internal.TypeHandlers.CompositeHandlers
{
    partial class CompositeHandler<T> : OpenGaussTypeHandler<T>, ICompositeHandler
    {
        readonly ConnectorTypeMapper _typeMapper;
        readonly IOpenGaussNameTranslator _nameTranslator;
        ...
        public CompositeHandler(PostgresCompositeType postgresType, ConnectorTypeMapper typeMapper, IOpenGaussNameTranslator nameTranslator)
            : base(postgresType)
        {
        ...
        public override ValueTask<T> Read(OpenGaussReadBuffer buffer, int length, bool async, FieldDescription? fieldDescription = null)
        {
            Initialize();
            ...
            async ValueTask<T> ReadUsingMemberHandlers(OpenGaussReadBuffer buffer, bool async)
            {
        ...
        public override async Task Write(T value, OpenGaussWriteBuffer buffer, OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
            Initialize();
        ...
        public override int ValidateAndGetLength(T value, ref OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter)
        {
            Initialize();
        ...
        static CompositeConstructorHandler<T>? CreateConstructorHandler(PostgresCompositeType pgType, ConnectorTypeMapper typeMapper, IOpenGaussNameTranslator nameTranslator)
        {
            ...
            foreach (var clrConstructor in clrType.GetConstructors())
            {
                ...
                for (var pgFieldIndex = 0; pgFieldIndex < pgFields.Count; ++pgFieldIndex)
                {
                    ...
                    if (!typeMapper.TryResolveByOID(pgField.Type.OID, out var handler))
                        throw new OpenGaussException($"PostgreSQL composite type {pgType.DisplayName} has field {pgField.Type.DisplayName} with an unknown type (OID = {pgField.Type.OID}).");
        ...
        static CompositeMemberHandler<T>[] CreateMemberHandlers(PostgresCompositeType pgType, ConnectorTypeMapper typeMapper, IOpenGaussNameTranslator nameTranslator)
        {
            var pgFields = pgType.Fields;
