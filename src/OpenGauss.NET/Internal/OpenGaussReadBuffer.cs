...
{
    /// <summary>
    /// A buffer used by OpenGauss to read data from the socket efficiently.
    /// Provides methods which decode different values types and tracks the current position.
    /// </summary>
    public sealed partial class OpenGaussReadBuffer : IDisposable
    {
        #region Fields and Properties

        public OpenGaussConnection Connection => Connector.Connection!;

        internal readonly OpenGaussConnector Connector;
                ...
                bool readingNotifications)
            {
                ...
                while (count > 0)
                {
                    ...
                    catch (Exception e)
                    {
                                                ...
                                                (Type.GetType("Mono.Runtime") == null ? SocketError.TimedOut : SocketError.WouldBlock):
                        {
                            ...
                            if (readingNotifications)
                            {
                                if (connector.UserCancellationRequested)
                                    throw;
                                throw OpenGaussTimeoutException();
                            }
                        ...
                        default:
                            throw connector.Break(new OpenGaussException("Exception while reading from stream", e));
                        }
                    }
                }

                buffer.Cts.Stop();
                OpenGaussEventSource.Log.BytesRead(totalRead);

                static Exception OpenGaussTimeoutException() => new OpenGaussException("Exception while reading from stream", TimeoutException());
        ...
        internal OpenGaussReadBuffer AllocateOversize(int count)
        {
            Debug.Assert(count > Size);
            var tempBuf = new OpenGaussReadBuffer(Connector, Underlying, _underlyingSocket, count, TextEncoding, RelaxedTextEncoding, usePool: true);
        ...
        public ValueTask<int> ReadAsync(Memory<byte> output, CancellationToken cancellationToken = default)
        {
            ...
            static async ValueTask<int> ReadAsyncLong(OpenGaussReadBuffer buffer, Memory<byte> output, CancellationToken cancellationToken)
            {
                ...
                {
                    throw buffer.Connector.Break(new OpenGaussException("Exception while reading from stream", e));
                }
            }
        }
        ...
        internal void CopyTo(OpenGaussReadBuffer other)
        {
