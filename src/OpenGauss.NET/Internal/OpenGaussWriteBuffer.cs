...
{
    /// <summary>
    /// A buffer used by OpenGauss to write data to the socket efficiently.
    /// Provides methods which encode different values types and tracks the current position.
    /// </summary>
    public sealed partial class OpenGaussWriteBuffer : IDisposable
    {
        ...
        public async Task Flush(bool async, CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception e)
            {
                ...
                case IOException _ when (e.InnerException as SocketException)?.SocketErrorCode ==
                    ...
                    throw Connector.Break(new OpenGaussException("Exception while writing to stream", new TimeoutException("Timeout during writing attempt")));
                }

                throw Connector.Break(new OpenGaussException("Exception while writing to stream", e));
            }
            OpenGaussEventSource.Log.BytesWritten(WritePosition);
            //OpenGaussEventSource.Log.RequestFailed();
        ...
        public Task WriteString(string s, int charLen, int byteLen, bool async, CancellationToken cancellationToken = default)
        {
            ...
            static async Task WriteStringLong(OpenGaussWriteBuffer buffer, bool async, string s, int charLen, int byteLen, CancellationToken cancellationToken)
            {
        ...
        internal Task WriteChars(char[] chars, int offset, int charLen, int byteLen, bool async, CancellationToken cancellationToken = default)
        {
            ...
            static async Task WriteCharsLong(OpenGaussWriteBuffer buffer, bool async, char[] chars, int offset, int charLen, int byteLen, CancellationToken cancellationToken)
            {
                Debug.Assert(byteLen > buffer.WriteSpaceLeft);
                if (byteLen <= buffer.Size)
                {
        ...
        public Task WriteBytesRaw(byte[] bytes, bool async, CancellationToken cancellationToken = default)
        {
            ...
            static async Task WriteBytesLong(OpenGaussWriteBuffer buffer, bool async, byte[] bytes, CancellationToken cancellationToken)
            {
