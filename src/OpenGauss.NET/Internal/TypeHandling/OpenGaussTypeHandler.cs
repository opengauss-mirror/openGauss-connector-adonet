...
namespace OpenGauss.NET.Internal.TypeHandling
{
    ...
    /// Type handler writers shouldn't inherit from this class, inherit <see cref="OpenGaussTypeHandler"/>
    /// or <see cref="OpenGaussSimpleTypeHandler{T}"/> instead.
    /// </summary>
    public abstract class OpenGaussTypeHandler
    {
        protected OpenGaussTypeHandler(PostgresType postgresType)
        ...
        protected internal async ValueTask<TAny> Read<TAny>(OpenGaussReadBuffer buf, int len, bool async, FieldDescription? fieldDescription = null)
        {
            switch (this)
            {
            case IOpenGaussSimpleTypeHandler<TAny> simpleTypeHandler:
                await buf.Ensure(len, async);
                return simpleTypeHandler.Read(buf, len, fieldDescription);
            case IOpenGaussTypeHandler<TAny> typeHandler:
        ...
        /// <summary>
        /// Version of <see cref="Read{TAny}(OpenGaussReadBuffer,int,bool,FieldDescription?)"/> that's called when we know the entire value
        /// is already buffered in memory (i.e. in non-sequential mode).
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public TAny Read<TAny>(OpenGaussReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            Debug.Assert(buf.ReadBytesLeft >= len);

            return this switch
            {
                IOpenGaussSimpleTypeHandler<TAny> simpleTypeHandler => simpleTypeHandler.Read(buf, len, fieldDescription),
                IOpenGaussTypeHandler<TAny> typeHandler => typeHandler.Read(buf, len, async: false, fieldDescription).Result,
                _ => ReadCustom<TAny>(buf, len, async: false, fieldDescription).Result
            };
        }

        protected internal virtual ValueTask<TAny> ReadCustom<TAny>(OpenGaussReadBuffer buf, int len, bool async, FieldDescription? fieldDescription)
        ...
        /// </summary>
        public abstract ValueTask<object> ReadAsObject(OpenGaussReadBuffer buf, int len, bool async, FieldDescription? fieldDescription = null);

        /// <summary>
        /// Version of <see cref="ReadAsObject(OpenGaussReadBuffer,int,bool,FieldDescription?)"/> that's called when we know the entire value
        /// is already buffered in memory (i.e. in non-sequential mode).
        /// </summary>
        internal object ReadAsObject(OpenGaussReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            Debug.Assert(buf.ReadBytesLeft >= len);
        ...
        /// memory, sync or async I/O will be performed as specified by <paramref name="async"/>.
        /// </summary>
        internal virtual ValueTask<object> ReadPsvAsObject(OpenGaussReadBuffer buf, int len, bool async, FieldDescription? fieldDescription = null)
            => ReadAsObject(buf, len, async, fieldDescription);

        /// <summary>
        /// Version of <see cref="ReadPsvAsObject(OpenGaussReadBuffer,int,bool,FieldDescription?)"/> that's called when we know the entire value
        /// is already buffered in memory (i.e. in non-sequential mode).
        /// </summary>
        internal virtual object ReadPsvAsObject(OpenGaussReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            Debug.Assert(buf.ReadBytesLeft >= len);
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal async ValueTask<TAny> ReadWithLength<TAny>(OpenGaussReadBuffer buf, bool async, FieldDescription? fieldDescription = null)
        {
        ...
        /// <summary>
        /// <para>Called to validate and get the length of a value of a generic <see cref="OpenGaussParameter{T}"/>.</para>
        ...
        protected internal int ValidateAndGetLength<TAny>(
            [DisallowNull] TAny value, [NotNullIfNotNull("lengthCache")] ref OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter)
        {
            Debug.Assert(value is not DBNull);

            return this switch
            {
                IOpenGaussSimpleTypeHandler<TAny> simpleTypeHandler => simpleTypeHandler.ValidateAndGetLength(value, parameter),
                IOpenGaussTypeHandler<TAny> typeHandler => typeHandler.ValidateAndGetLength(value, ref lengthCache, parameter),
        ...
        /// <summary>
        /// Called to write the value of a generic <see cref="OpenGaussParameter{T}"/>.
        /// </summary>
        /// <summary>
        /// In the vast majority of cases writing a parameter to the buffer won't need to perform I/O.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public async Task WriteWithLength<TAny>(TAny? value, OpenGaussWriteBuffer buf, OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
        ...
        /// The <see cref="OpenGaussParameter"/> instance where this value resides. Can be used to access additional
        /// information relevant to the write process (e.g. <see cref="OpenGaussParameter.Size"/>).
        /// </param>
        /// <returns>The number of bytes required to write the value.</returns>
        // Source-generated
        public abstract int ValidateObjectAndGetLength(object value, ref OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter);
        ...
        /// </param>
        // Source-generated
        public abstract Task WriteObjectWithLength(object? value, OpenGaussWriteBuffer buf, OpenGaussLengthCache? lengthCache, OpenGaussParameter? parameter, bool async, CancellationToken cancellationToken = default);
        ...
        /// </summary>
        public abstract OpenGaussTypeHandler CreateArrayHandler(PostgresArrayType pgArrayType, ArrayNullabilityMode arrayNullabilityMode);

        /// <summary>
        /// Creates a type handler for ranges of this handler's type.
        /// </summary>
        public abstract OpenGaussTypeHandler CreateRangeHandler(PostgresType pgRangeType);

        /// <summary>
        /// Creates a type handler for multiranges of this handler's type.
        /// </summary>
        public abstract OpenGaussTypeHandler CreateMultirangeHandler(PostgresMultirangeType pgMultirangeType);

        /// <summary>
        /// Used to create an exception when the provided type can be converted and written, but an
        /// instance of <see cref="OpenGaussParameter"/> is required for caching of the converted value
        /// (in <see cref="OpenGaussParameter.ConvertedValue"/>.
        /// </summary>
