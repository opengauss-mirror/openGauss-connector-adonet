    ...
    public static class ClusterStateCache
    {
        ...
        internal static ClusterState UpdateClusterState(string host, int port, ClusterState state, DateTime timeStamp, TimeSpan stateExpiration)
            => Clusters.AddOrUpdate(
                ...
                new ClusterInfo(state, new OpenGaussTimeout(stateExpiration), timeStamp),
                (_, oldInfo) => oldInfo.TimeStamp >= timeStamp ? oldInfo : new ClusterInfo(state, new OpenGaussTimeout(stateExpiration), timeStamp)).State;
#else
        internal static ClusterState UpdateClusterState(string host, int port, ClusterState state, DateTime timeStamp, TimeSpan stateExpiration)
            => Clusters.AddOrUpdate(
                new ClusterIdentifier(host, port),
                ...
                new ClusterInfo(state, new OpenGaussTimeout(stateExpiration), timeStamp)).State;
#endif
        ...
        readonly struct ClusterInfo
        {
            ...
            internal readonly OpenGaussTimeout Timeout;
            ...
            public ClusterInfo(ClusterState state, OpenGaussTimeout timeout, DateTime timeStamp)
