...
namespace OpenGauss.NET
{
    ...
    /// </summary>
    public class OpenGaussLargeObjectManager
    {
        const int InvWrite = 0x00020000;
        const int InvRead = 0x00040000;

        internal OpenGaussConnection Connection { get; }
        ...
        /// <summary>
        /// Creates an OpenGaussLargeObjectManager for this connection. The connection must be opened to perform remote operations.
        /// </summary>
        /// <param name="connection"></param>
        public OpenGaussLargeObjectManager(OpenGaussConnection connection)
        {
        ...
        {
            using var command = new OpenGaussCommand(function, Connection)
            {
            ...
            foreach (var argument in arguments)
                command.Parameters.Add(new OpenGaussParameter { Value = argument });
        ...
        /// <returns>An OpenGaussLargeObjectStream</returns>
        public OpenGaussLargeObjectStream OpenRead(uint oid)
        ...
        /// </param>
        /// <returns>An OpenGaussLargeObjectStream</returns>
        public Task<OpenGaussLargeObjectStream> OpenReadAsync(uint oid, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
                return OpenRead(oid, true, cancellationToken);
        }

        async Task<OpenGaussLargeObjectStream> OpenRead(uint oid, bool async, CancellationToken cancellationToken = default)
        {
            var fd = await ExecuteFunction<int>("lo_open", async, cancellationToken, (int)oid, InvRead);
            return new OpenGaussLargeObjectStream(this, fd, false);
        }
        ...
        /// <param name="oid">Oid of the object</param>
        /// <returns>An OpenGaussLargeObjectStream</returns>
        public OpenGaussLargeObjectStream OpenReadWrite(uint oid)
        ...
        /// </param>
        /// <returns>An OpenGaussLargeObjectStream</returns>
        public Task<OpenGaussLargeObjectStream> OpenReadWriteAsync(uint oid, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
                return OpenReadWrite(oid, true, cancellationToken);
        }

        async Task<OpenGaussLargeObjectStream> OpenReadWrite(uint oid, bool async, CancellationToken cancellationToken = default)
        {
            var fd = await ExecuteFunction<int>("lo_open", async, cancellationToken, (int)oid, InvRead | InvWrite);
            return new OpenGaussLargeObjectStream(this, fd, true);
        }
